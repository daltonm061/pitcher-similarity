...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Read sort from query params (set by JS onclick in table header) ────────
    qp = st.query_params
    if "lb_sort" in qp:
        new_col = qp["lb_sort"]
        new_asc = qp.get("lb_asc", "0") == "1"
        if new_col != st.session_state.get("lb_sort_col") or new_asc != st.session_state.get("lb_sort_asc", False):
            st.session_state["lb_sort_col"] = new_col
            st.session_state["lb_sort_asc"] = new_asc
            # Clear query params then rerun cleanly
            st.query_params.clear()
            st.rerun()

    sort_by  = st.session_state.get("lb_sort_col", "Velo")
    sort_asc = st.session_state.get("lb_sort_asc", False)

    # ── Data rows ─    # ── Data rows ─────────────────────────────────────────────────────────────
    # ── Build clickable headers + JS sort function ───────────────────────────
    js_sort_fn = (
        "<script>"
        "function lbSort(col,curCol,curAsc){"
        "var a=(col===curCol)?(curAsc==='1'?'0':'1'):'0';"
        "var u=new URL(window.location.href);"
        "u.searchParams.set('lb_sort',col);"
        "u.searchParams.set('lb_asc',a);"
        "window.location.href=u.toString();}"
        "</script>"
    )
    asc_str = "1" if sort_asc else "0"
    header_cells = ""
    for col in DISPLAY_COLS:
        ...
        onclick = f"lbSort('{col}','{sort_by}','{asc_str}')"
        header_cells += (
            f"<th onclick='{onclick}' "
            f"style='padding:8px 8px;text-align:{align};"
            f"font-family:JetBrains Mono,monospace;font-size:9px;color:{color};"
            f"text-transform:uppercase;letter-spacing:1px;"
            f"background:{bg};border-bottom:{bdr};"
            f"white-space:nowrap;cursor:pointer;user-select:none;font-weight:{fw}'>"
            f"{col}{arrow}</th>"
        )
    ...
    table_html = (
        js_sort_fn +
        "<div style='overflow-x:auto;border:1px solid #1a2a40;border-radius:10px'>"
