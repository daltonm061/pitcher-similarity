...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Apply range filters from session state ────────────────────────────────
    for col in filter_cols:
        mn = st.session_state.get(f"lb_min_{col}")
        mx = st.session_state.get(f"lb_max_{col}")
        # Only filter if value is a real number (not None, not NaN)
    ...
    for j, col in enumerate(CTRL_COLS):
        with row2[j+1]:
            # Don't pass value= at all — let Streamlit manage state via key only
            # This prevents value=None from resetting the widget on each rerun
            if f"lb_max_{col}" not in st.session_state:
                st.session_state[f"lb_max_{col}"] = None
            st.number_input(
                " ",
                key=f"lb_max_{col}",
                label_visibility="collapsed",
    ...
    for j, col in enumerate(CTRL_COLS):
        with row3[j+1]:
            if f"lb_min_{col}" not in st.session_state:
                st.session_state[f"lb_min_{col}"] = None
            st.number_input(
                " ",
                key=f"lb_min_{col}",
                label_visibility="collapsed",
