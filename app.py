...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Sort from dataframe column header click ──────────────────────────────
    # st.dataframe stores sort state in session_state[key]['sorting']
    df_state = st.session_state.get("lb_df_sel", {})
    sorting  = df_state.get("sorting", []) if isinstance(df_state, dict) else []
    if sorting:
        sort_col = sorting[0].get("column_name")
        sort_asc = sorting[0].get("direction", "desc") == "asc"
        if sort_col and sort_col in view.columns:
            view = view.sort_values(sort_col, ascending=sort_asc, na_position="last")
    else:
        # Default sort: Velo descending
    ...
    st.markdown(
        ...
        f"{total_rows:,} pitches · showing top {min(500,total_rows):,} — "
    ...
    # Apply gradient styling via pandas Styler
    def _grad(col):
        if col.name not in lb_baselines:
            return [""] * len(col)
        mu, sd = lb_baselines[col.name]
        invert = col.name in ("xwOBA",)
        styles = []
        for val in col:
            if val is None or (isinstance(val, float) and val != val):
                styles.append("background-color:#141e2e;color:#7aaac0")
                continue
            bg, txt = cell_color(col.name, val,
                                  pitch_type=None,
                                  hand=None)
            styles.append(f"background-color:{bg};color:{txt}")
        return styles

    gradient_cols = [c for c in ["Velo","iVB","HB","VAA","HAA","Stuff+",
                                  "CSW%","Whiff%","xwOBA"] if c in df_show.columns]

    styled = df_show.style
    for col in gradient_cols:
        styled = styled.apply(_grad, subset=[col])
    ...
    init_sel = {"rows": [cur_idx]} if 0 <= cur_idx < len(df_show) else {"rows": []}
