...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Apply range filters from _lb_filters shadow dict ─────────────────────
    _filters = st.session_state.get("_lb_filters", {})
    for col in filter_cols:
        mn = _filters.get(f"min_{col}")
        mx = _filters.get(f"max_{col}")
        if mn is not None and mn == mn and col in view.columns:
            view = view[view[col].fillna(-9999) >= float(mn)]
        if mx is not None and mx == mx and col in view.columns:
    ...
    if "_lb_filters" not in st.session_state:
        st.session_state["_lb_filters"] = {}
    ...
    for j, col in enumerate(CTRL_COLS):
        with row2[j+1]:
            cur = st.session_state["_lb_filters"].get(f"max_{col}", None)
            new_val = st.number_input(
                " ", value=cur, key=f"lb_max_{col}_widget",
                label_visibility="collapsed", format="%.1f", step=0.1,
            )
            # Only store if user typed a value (not the 0.0 default on blank)
            if new_val != cur:
                st.session_state["_lb_filters"][f"max_{col}"] = new_val
    ...
    for j, col in enumerate(CTRL_COLS):
        with row3[j+1]:
            cur = st.session_state["_lb_filters"].get(f"min_{col}", None)
            new_val = st.number_input(
                " ", value=cur, key=f"lb_min_{col}_widget",
                label_visibility="collapsed", format="%.1f", step=0.1,
            )
            if new_val != cur:
                st.session_state["_lb_filters"][f"min_{col}"] = new_val
    ...
    if "lb_hm_row" not in st.session_state:
        st.session_state["lb_hm_row"] = "— select a pitch —"

    sel_row = st.session_state.get("lb_hm_row", "— select a pitch —")

    if sel_row != "— select a pitch —":
        # Guard: selected row may no longer exist after filtering
        if sel_row not in row_labels:
            st.session_state["lb_hm_row"] = "— select a pitch —"
            sel_row = "— select a pitch —"
    ...
    # ── CSS: shrink map buttons to match table row height exactly ────────────
    st.markdown("""
    <style>
    /* Scope to the map button column only via a wrapper div */
    .lb-map-col div[data-testid="stVerticalBlock"] {
        gap: 0px !important;
    }
    .lb-map-col button {
        ...
        border-bottom: 1px solid #0f1820 !important;
    ...
    .lb-map-col button:hover {
        background: #162236 !important;
        color: #d4a848 !important;
    }
    .lb-map-col button[kind="primary"] {
        background: #162236 !important;
        color: #d4a848 !important;
        border-left: 2px solid #d4a848 !important;
    }
    /* Header spacer in map col */
    .lb-map-col .lb-map-header {
        height: 33px;
        display: flex;
        align-items: center;
    ...
    # ── Render table + per-row heatmap buttons side by side ─────────────────
    btn_col, tbl_col = st.columns([0.4, 11])

    with btn_col:
        st.markdown("<div class='lb-map-col'>", unsafe_allow_html=True)
        st.markdown("<div class='lb-map-header'>MAP</div>", unsafe_allow_html=True)
        for idx_r, label in enumerate(row_labels):
            row_data = view_display.iloc[idx_r]
            is_selected = (st.session_state.get("lb_hm_row") == label)
            if st.button(
                "▶" if is_selected else "🗺",
                key=f"lb_row_btn_{idx_r}",
                use_container_width=True,
                type="primary" if is_selected else "secondary",
                help=f"{row_data['Pitcher']} {row_data['Year']} — {row_data['Pitch']}",
            ):
                st.session_state["lb_hm_row"] = "— select a pitch —" if is_selected else label
                st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
