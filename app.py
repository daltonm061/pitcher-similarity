...
/* Pitch metric text inputs (replaces number_input to allow single-click clear) */
/* Hide native browser clear button — we use our own per-card clear */
.stTextInput > div > div > input::-webkit-search-cancel-button,
.stTextInput > div > div > input::-webkit-clear-button,
[data-testid="stTextInputRootElement"] [data-testid="stTextInputClearButton"] {
    display: none !important;
}
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with main_col:
        ...
        def pitch_inputs_widget(group, key_prefix, tm_data, show_placeholder=False):
            ...
            with _cc:
                def _clear_widget():
                    st.session_state[f"{key_prefix}_velo"] = ""
                    st.session_state[f"{key_prefix}_ivb"]  = ""
                    st.session_state[f"{key_prefix}_hb"]   = ""
...
"Use the dropdown above to add your pitches.</div>",
            ...
            with sp_col_w:
                ...
                with _sp_cc:
                    def _clear_sp():
                        st.session_state["sp_velo"] = ""
                        st.session_state["sp_ivb"]  = ""
                        st.session_state["sp_hb"]   = ""
                    st.markdown("<div style='margin-top:18px'></div>", unsafe_allow_html=True)
