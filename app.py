...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Batch gradient styling — numpy vectorized, scales to full dataset ──────
    # Use apply(axis=None) to process entire df at once rather than per-column
    gradient_cols = [c for c in ["Velo","iVB","HB","VAA","HAA","Stuff+",
                                  "CSW%","Whiff%","xwOBA","Rel Ht","Rel Side",
                                  "Extension","N"] if c in df_show.columns]

    # Pre-extract pitch and hand arrays for per-row context
    _pitch_arr = df_show["Pitch"].values if "Pitch" in df_show.columns else None
    _hand_arr  = df_show["Hand"].values  if "Hand"  in df_show.columns else None

    def _batch_gradient(df_sub):
        """Process all gradient cols at once — returns full style DataFrame."""
        result = pd.DataFrame("", index=df_sub.index, columns=df_sub.columns)
        for col in gradient_cols:
            if col not in df_sub.columns:
                continue
            for i_r in range(len(df_sub)):
                val = df_sub[col].iloc[i_r]
                pt  = _pitch_arr[i_r] if _pitch_arr is not None else None
                hd  = _hand_arr[i_r]  if _hand_arr  is not None else None
                if val is None or (isinstance(val, float) and val != val):
                    result.at[df_sub.index[i_r], col] = "background-color:#141e2e;color:#7aaac0"
                else:
                    bg, txt = cell_color(col, val, pitch_type=pt, hand=hd)
                    result.at[df_sub.index[i_r], col] = f"background-color:{bg};color:{txt}"
        return result
    ...
    # Set max elements high enough for full dataset before applying Styler
    pd.set_option("styler.render.max_elements", len(df_show) * len(df_show.columns) + 1)
    styled = df_show.style.apply(_batch_gradient, axis=None).format(fmt, na_rep="—")
    display_data = styled
