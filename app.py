...
def run_search_single_pitch(user, velo, ivb, hb_csv, top_n, pitch_type_filter=None):
    ...
    for _, r in profiles.iterrows():
        ...
        for group in search_groups:
            ...
            # Strict greater-than: ties keep the first group encountered
            # (PITCH_GROUPS order: 4-Seam first, ensuring 4-seam wins ties)
        ...
        if multi_pitch_mode:
            # Append ALL pitch types that scored above a floor, ordered by score
            for group in search_groups:
                ...
                for metric, mv, sigma in [
                    ...
                    ("ivb",  r.get(f"ivb_{group}"),  SIGMA["ivb"]),
                    ("hb",   r.get(f"hb_{group}"),   SIGMA["hb"]),
                ]:
                    user_val = {"velo": velo, "ivb": ivb, "hb": hb_csv}[metric]
                    if user_val is None:
                        continue
                ...
                if score < 20:
                    continue  # skip clearly irrelevant pitches
                rows.append({
                    "Similarity":   score,
                    "Pitcher":      r["player_name"],
                    "Year":         int(r["year"]),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with lc:
        st.markdown(
            ...
            "color:#c9a84c;letter-spacing:3px;text-transform:uppercase;margin-bottom:20px'>"
            "⚾  Searching…</div>"
            "<div style='font-family:monospace;font-size:11px;color:#3a6a8a'>"
            "Scoring against 1,700+ pitcher seasons</div>"
            "</div>",
            unsafe_allow_html=True,
        )
    ...
    for idx, r in enumerate(results):
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            def pitch_sort_key(g):
                if result_mode == "single":
                    return -pitch_similarity_score(g)   # most similar first
                return -pitch_pct(g)                    # highest usage first
