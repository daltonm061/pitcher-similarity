...
def parse_trackman_image(file_bytes: bytes, filename: str) -> dict:
    ...
    Extracts text from the image then feeds it through the same regex/line
    parsing logic as the PDF parser.
    ...
    try:
        ...
        # Pre-process for better OCR accuracy on report screenshots:
        # 1. Scale up small images (Tesseract works best at ~300dpi)
        w, h = img.size
        if w < 1200:
            scale = 1200 / w
            img = img.resize((int(w * scale), int(h * scale)), Image.LANCZOS)

        # 2. Convert to grayscale, sharpen, boost contrast
        img = img.convert("L")
        img = img.filter(ImageFilter.SHARPEN)
        img = ImageEnhance.Contrast(img).enhance(2.0)

        # 3. Run Tesseract — PSM 6 = assume uniform block of text (good for tables)
    ...
    # ── Reuse PDF text-parsing logic verbatim ────────────────────────────────
    import re

    text_lines = raw_text.split("\n")
    # Normalize Unicode minus signs
    text_lines = [l.replace("\u2212", "-") for l in text_lines]
    # Remove thousand-separator commas (e.g. 2,346 → 2346)
    text_lines = [re.sub(r"(\d),(\d{3})\b", r"\1\2", l) for l in text_lines]
    ...
    for line in text_lines:
        ...
        # Drop pitch count (small integers) and spin rate (> 500)
        metric_nums = [
            f for f in floats
