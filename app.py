...
_tj_model = load_tjstuff_model()

# Exact 10 features extracted from model binary
_TJSP_FEATURES = ["start_speed","spin_rate","extension","az","ax","x0","z0",
...
def score_tjstuff(pitches: dict, rel_height: float = 5.74,
    ...
    p_speed = primary[1].get("velo") or _TJSP_MEDIANS["start_speed"]
    _p_az_raw = primary[1].get("az")
    _p_ax_raw = primary[1].get("ax")
    p_az    = float(_p_az_raw) if (_p_az_raw is not None and not (isinstance(_p_az_raw, float) and math.isnan(_p_az_raw))) else _TJSP_MEDIANS["az"]
    _p_ax_r = float(_p_ax_raw) if (_p_ax_raw is not None and not (isinstance(_p_ax_raw, float) and math.isnan(_p_ax_raw))) else _TJSP_MEDIANS["ax"]
    ...
    for grp, m in pitches.items():
        ...
        if velo is None:
            continue
        az   = m.get("az") if m.get("az") is not None else _TJSP_MEDIANS["az"]
        ax_r = m.get("ax") if m.get("ax") is not None else _TJSP_MEDIANS["ax"]
        # Normalize ax to arm-side negative convention (RHP as-is, flip LHP)
        ax   = ax_r if hand == "R" else -ax_r
        rows.append({
            "start_speed": velo,
            "spin_rate":   m.get("spin_rate") or _TJSP_MEDIANS["spin_rate"],
            "extension":   extension or _TJSP_MEDIANS["extension"],
            "az":          az,
            "ax":          ax,
            "x0":          x0,
            "z0":          rel_height or _TJSP_MEDIANS["z0"],
    ...
    X = _pd.DataFrame(rows)[_TJSP_FEATURES].fillna(0)
    try:
        xrv = _tj_model.predict(X) * 100  # model outputs xRV/pitch → xRV/100
    except Exception:
        return {}
    xrv_mean, xrv_sd = 0.0, 0.68
    tjsp = 100.0 - ((xrv - xrv_mean) / xrv_sd) * 10.0
    return {k: round(float(v), 1) for k, v in zip(keys, tjsp)}
...
try:
    ...
    # Compute weighted xwOBA per pitch group (filtered to stand='all')
    _xw_grp = (_zs_all.assign(xw_weighted=_zs_all["xwoba_mean"].fillna(0) * _zs_all["n_pitches"])
               .groupby("pitch_group")
               .apply(lambda g: g["xw_weighted"].sum() / g["n_pitches"].sum(), include_groups=False)
               .rename("xw_mu_w"))
    pitch_grp_league = (
...
def comp_aggregate_stats(results, pitch_group=None):
    ...
    for r in results:
        ...
        hand_r = r.get("Hand", "R")
        # Convert HB to arm-side convention before averaging (positive = arm-side)
        hb_display = (-float(hb) if hand_r == "R" else float(hb)) if hb is not None and not (isinstance(hb, float) and hb != hb) else None
        for k, v in [("velo",velo),("ivb",ivb),("hb",hb_display),("vaa",vaa),("haa",haa),("stuff_plus",sp)]:
        ...
        if zone_stats_ok and not zone_stats.empty:
            ...
            if not sub.empty:
                ...
                # Weight by n_pitches, divide by total pitches (not just contact pitches)
                # This treats non-contact pitches as xwOBA=0, matching Statcast methodology
                _total_n = sub["n_pitches"].sum()
                if _total_n > 0:
                    _xw_num = (sub["xwoba_mean"].fillna(0) * sub["n_pitches"]).sum()
                    vals["xwoba"].append(_xw_num / _total_n)
...
def score_row(user, pitch_inputs, row):
    ...
    for key in ("rel_height", "rel_side", "extension"):
        ...
        if not is_real(mv):
            ...
            # Direct comparison — rel_side already arm-side normalized
    ...
    for r in _pdicts:
        ...
        for key in ("rel_height", "rel_side", "extension"):
            ...
            if not is_real(mv):
                ...
                # rel_side: compare directly — arm-side convention preserved
                # abs() would lose submarine vs over-the-top distinction within same hand
                ref = val
                cmp = mv
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    prog.progress(80, text="Computing TJStuff+…")

    # ── Score user's entered pitches through TJStuff+ model ──────────────────
    ...
    _user_tjsp = {}
    if _tj_model is not None and data_ok:
        try:
            import math as _m
            ...
            def _infer_az_ax(velo, ivb_in, hb_in):
                ...
                if velo is None or velo <= 0:
                    return _TJSP_MEDIANS["az"], _TJSP_MEDIANS["ax"]
                t   = 55.0 / (velo * 1.467)
                t2h = t * t / 2.0
                az  = (ivb_in / 12.0) / t2h - 32.174 if ivb_in is not None else _TJSP_MEDIANS["az"]
                ax  = (hb_in  / 12.0) / t2h           if hb_in  is not None else _TJSP_MEDIANS["ax"]
            ...
            if mode == "arsenal":
                for _grp, _pm in snap["pitch_inputs"].items():
                    ...
                    _score_pitches[_grp] = {
                        "velo": _velo, "az": _az, "ax": _ax, "spin_rate": _TJSP_MEDIANS["spin_rate"],
                    }
            else:
            ...
            _rh  = snap["user"].get("rel_height") or _TJSP_MEDIANS["z0"]
            _rs  = snap["user"].get("rel_side") or (-1.89 if _hand == "R" else 2.08)
            _ext = snap["user"].get("extension")  or _TJSP_MEDIANS["extension"]

            if _score_pitches:
                _user_tjsp = score_tjstuff(
                    _score_pitches,
                    rel_height=_rh,
                    rel_side=_rs,
                    extension=_ext,
            ...
            print(f"[TJStuff+ scoring error] {_e}\n{_tb.format_exc()}")
            _user_tjsp = {}

    prog.progress(100, text="Done!")
    st.session_state.results      = results
    st.session_state.user_tjsp    = _user_tjsp
    ...
    with main_col:
        ...
        def pitch_inputs_widget(group, key_prefix, tm_data, show_placeholder=False):
            ...
            ivb = st.number_input(" ", min_value=-30.0, max_value=30.0,
                                  ...
                                  label_visibility="collapsed")
            st.markdown("<div class='field-label'>HB — arm-side positive (in)</div>", unsafe_allow_html=True)
        ...
        if run:
            ...
            _rs_raw = vn(rel_side_v)
            # Convert arm-side positive → Statcast convention (RHP negative, LHP positive)
            if _rs_raw is not None and _hand_key == "R":
                _rs_csv = -abs(_rs_raw)
            elif _rs_raw is not None and _hand_key == "L":
                _rs_csv = abs(_rs_raw)
            else:
                _rs_csv = _rs_raw  # unknown hand: pass through as-is
            user = {
                "hand":       _hand_key,
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Filter controls ───────────────────────────────────────────────────────
    METRIC_COLS = ["Velo","iVB","HB","VAA","HAA","Stuff+","CSW%","Whiff%","xwOBA","Rel Ht","Rel Side","Extension","N"]
    ...
    lb_vaa_haa = _vaa_haa_league  # already keyed by PITCH_GROUP
    ...
    def cell_color(col, val, pitch_type=None, hand=None):
        ...
        # VAA/HAA: outlier coloring per pitch type (blue=avg, red=extreme either dir)
        if col in ("VAA", "HAA") and pitch_type:
            # Prefer hand-specific baseline (HAA differs significantly by hand)
            _vl = (lb_vaa_haa.get(f"{pitch_type}_{hand}") or
                   lb_vaa_haa.get(pitch_type, {}))
            if _vl:
                if col == "VAA":
                    return _outlier_color(val, _vl["vaa_mu"], _vl["vaa_sd"])
                else:
                    return _outlier_color(val, _vl["haa_mu"], _vl["haa_sd"])
        if col in ("VAA", "HAA"):
        ...
        # Rel Ht / Rel Side / Extension: outlier coloring normalized by handedness
    ...
    # ── Row 2: MAX inputs ─────────────────────────────────────────────────────
    def _cb_max(c):
        st.session_state["_lbf"][f"max_{c}"] = st.session_state.get(f"_w_max_{c}")
    def _cb_min(c):
        st.session_state["_lbf"][f"min_{c}"] = st.session_state.get(f"_w_min_{c}")
    def _cb_clear_max(c):
    ...
    for j, col in enumerate(CTRL_COLS):
        with row_max[j+1]:
            ...
            if has_max:
                _in, _clr = st.columns([5, 1])
            else:
                _in = st.container()
            with _in:
                st.number_input(
                    " ", value=st.session_state["_lbf"].get(f"max_{col}"),
                    key=f"_w_max_{col}", on_change=_cb_max, args=(col,),
                    label_visibility="collapsed", format="%.1f", step=0.1,
                )
            if has_max:
                with _clr:
                    st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)
                    if st.button("×", key=f"_clr_max_{col}", help=f"Clear MAX {col}",
                                 width='stretch'):
                        _cb_clear_max(col)
                        st.rerun()
    ...
    for j, col in enumerate(CTRL_COLS):
        with row_min[j+1]:
            ...
            with _in:
                st.number_input(
                    " ", value=st.session_state["_lbf"].get(f"min_{col}"),
                    key=f"_w_min_{col}", on_change=_cb_min, args=(col,),
                    label_visibility="collapsed", format="%.1f", step=0.1,
                )
            if has_min:
                with _clr:
                    st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)
                    if st.button("×", key=f"_clr_min_{col}", help=f"Clear MIN {col}",
                                 width='stretch'):
                        _cb_clear_min(col)
                        st.rerun()
    ...
    def _build_style_df(profiles_hash: int, df_json: str,
        ...
        _df = pd.read_json(_json.loads(df_json), orient="split")

        gradient_cols = [c for c in ["Velo","iVB","HB","VAA","HAA","Stuff+",
        ...
        for _col in gradient_cols:
            ...
            if _col in ("VAA","HAA"):
                for _i in _np.where(_valid)[0]:
                    ...
                    _vl = (_vaa_haa_league.get(f"{_pt}_{_hd}", {}) or
                           _vaa_haa_league.get(_pt, {})) if _pt else {}
                    if _vl:
                        _key = "vaa_mu" if _col=="VAA" else "haa_mu"
                        _sdk = "vaa_sd" if _col=="VAA" else "haa_sd"
                        _bg, _tx = _outlier_color(_vals[_i], _vl.get(_key,0), max(_vl.get(_sdk,1),0.001))
    ...
    _full_json  = _json.dumps(_full_disp.to_json(orient="split"))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    user_tjsp     = st.session_state.get("user_tjsp", {})
    ...
    def stat_pill(label, val, color, sub=None):
        """Unified pill: vibrant solid bg, compact layout."""
        sub_html = (
            f"<div style='font-family:monospace;font-size:8px;"
            f"color:#00000099;margin-top:2px;letter-spacing:0.5px'>{sub}</div>"
        ) if sub else ""
        return (
            f"<div style='text-align:center;background:{color};"
            f"border-radius:7px;padding:6px 4px;filter:brightness(1.25)'>"
            f"<div style='font-family:JetBrains Mono,monospace;font-size:8px;"
            f"color:#000000cc;text-transform:uppercase;letter-spacing:1px;"
            f"margin-bottom:3px;font-weight:700'>{label}</div>"
            f"<div style='font-family:Inter,sans-serif;font-size:17px;"
    ...
    def render_comp_section(title_label, title_color, groups_to_show):
        ...
        for grp in groups_to_show:
            ...
            def _fv_hb():
                """HB for display: positive = arm-side for the comp group's hand."""
                if "hb" not in agg: return "—"
                raw, _ = agg["hb"]
                # Use handedness of pitchers in this pitch group
                grp_hands = [r.get("Hand","R") for r in results
                             if r.get("Matched Pitch") == grp or not grp]
                # HB is already in arm-side convention from comp_aggregate_stats
                return f"{raw:+.1f}"
            ...
            # Use pitch-group-specific baselines when available
            ...
            _xw_sd   = 0.06
            csw_c   = stat_gradient_color(csw_val, _csw_mu, _csw_sd) if csw_val else "#4a6a80"
            xw_c    = stat_gradient_color(xw_val,  _xw_mu,  _xw_sd, invert=True) if xw_val else "#4a6a80"
            whiff_val, _ = agg.get("whiff", (None, 0))
            whiff_c = stat_gradient_color(whiff_val, 0.22, 0.04) if whiff_val else "#4a6a80"

            # VAA/HAA gradient vs pitch-type league average (hand-specific)
            _grp_hands = [r.get("Hand","R") for r in results
                          if r.get("Matched Pitch") == grp or not grp]
            _dom_hand = "R" if not _grp_hands or _grp_hands.count("R") >= _grp_hands.count("L") else "L"
            _vl = (_vaa_haa_league.get(f"{grp}_{_dom_hand}") or
                   _vaa_haa_league.get(grp, {}))
            ...
            # 3×3 pill grid: Row1=Velo/iVB/HB, Row2=VAA/HAA/Stuff+, Row3=Whiff%/CSW%/xwOBA
            pills_html = (
                ...
                + stat_pill("TJ S+",
                            fv("stuff_plus","i") if sp_val else (f"{user_tjsp[grp]:.0f}" if grp in user_tjsp else "—"),
                            sp_c if sp_val else (stuff_color(user_tjsp[grp]) if grp in user_tjsp else "#3a4a5a"),
                            (f"{user_tjsp[grp]:.0f} AVG" if grp in user_tjsp
                             else (stuff_grade_label(sp_val) if sp_val else "—")))
    ...
    for idx, r in enumerate(results):
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            # Robust Stuff+ lookup — handles numpy float64 NaN from dict(row)
            fg_overall = None
            ...
            strip = (
                ...
                f"<span>SIDE <b style='color:#8aadcc'>{abs(r['Rel Side']):.2f} ft</b></span>"
                ...
                "letter-spacing:1.2px;font-weight:500'>TJ Stuff+</span>"
            ...
            if not active:
                ...
                def sub_label(val, color_hex="#5a8aaa"):
                    if val:
                        return (
                            ...
                            f"font-weight:600;color:#00000099;margin-top:3px;"
                            f"background:#ffffff22;border-radius:3px;padding:1px 4px;"
                            f"display:inline-block'>{val}</div>"
                        )
                    return ""
                ...
                for group in active:
                    ...
                    _vl = (_vaa_haa_league.get(f"{group}_{hand}") or
                           _vaa_haa_league.get(group, {}))
                    ...
                    u_velo = f"{user_m['velo']:.1f} AVG" if user_m.get("velo") is not None else ""
                    u_ivb  = f"{user_m['ivb']:.1f}\" AVG" if user_m.get("ivb") is not None else ""
                    u_hb   = f"{-user_m['hb']:.1f}\" AVG" if user_m.get("hb") is not None else ""
                    ...
                    for _sp_key in ([sp_col] if sp_col else []) + ["stuff_plus"]:
                        ...
                        try:
                            _sp_f = float(_sp_v)
                            if math.isnan(_sp_f) or _sp_f <= 0:
                                continue
                            sp_val = _sp_f
                            break
                        except (TypeError, ValueError):
                    ...
                    if not pz_data.empty and "n_pitches" in pz_data.columns:
                        ...
                        _total_n_xw = pz_data["n_pitches"].sum()
                        if _total_n_xw > 0:
                            p_xwoba = float((pz_data["xwoba_mean"].fillna(0) * pz_data["n_pitches"]).sum() / _total_n_xw)
                    ...
                    if p_csw is None and zone_stats_ok and not zone_stats.empty:
                        ...
                        if not sub.empty:
                            ...
                            _total_n_fb = sub["n_pitches"].sum()
                            if _total_n_fb > 0:
                                p_xwoba = float((sub["xwoba_mean"].fillna(0) * sub["n_pitches"]).sum() / _total_n_fb)
                    ...
                    def _pc(lbl, val, c, sub=None):
                        """Same unified pill style as stat_pill."""
                        sub_h = (
                            f"<div style='font-family:monospace;font-size:8px;"
                            f"color:#00000099;margin-top:2px;letter-spacing:0.5px'>{sub}</div>"
                        ) if sub else ""
                        return (
                            f"<div style='text-align:center;background:{c};"
                            f"border-radius:7px;padding:6px 4px;filter:brightness(1.25)'>"
                            f"<div style='font-family:JetBrains Mono,monospace;font-size:8px;"
                            f"color:#000000cc;text-transform:uppercase;letter-spacing:1px;"
                            f"margin-bottom:3px;font-weight:700'>{lbl}</div>"
                            f"<div style='font-family:Inter,sans-serif;font-size:17px;"
                            f"font-weight:700;color:#000000;line-height:1'>{val}</div>"
                            f"{sub_h}</div>"
                        )
                    ...
                    card_html = (
                        ...
                        f"<span>● {group}</span>"
                        f"<span style='font-size:10px;color:#7aaac0;font-weight:400'>{pct_str} · {n_str}</span>"
                        f"</div>"
                        # 3×3 grid
                        "<div style='display:grid;grid-template-columns:repeat(3,1fr);gap:6px'>"
                        # Row 1: Velo | iVB | HB
                        + _pc("VELO",   velo_s,   color,    sub_label(u_velo) if u_velo else None)
                        + _pc("iVB",    ivb_s,    "#8aadcc", sub_label(u_ivb)  if u_ivb  else None)
                        + _pc("HB",     hb_s,     "#8aadcc", sub_label(u_hb)   if u_hb   else None)
                        # Row 2: VAA | HAA | Stuff+
                        + _pc("VAA",    vaa_s,    vaa_gc)
                        + _pc("HAA",    haa_s,    haa_gc)
                        + _pc("TJ S+",
                               sp_str if sp_has else (f"{user_tjsp[group]:.0f}" if group in user_tjsp else "—"),
                               sp_color if sp_has else (stuff_color(user_tjsp[group]) if group in user_tjsp else "#3a6a8a"),
                               (f"{user_tjsp[group]:.0f} AVG" if group in user_tjsp
                                else (sp_lbl if sp_has else "—")))
                        # Row 3: Whiff% | CSW% | xwOBA
                        + _pc("Whiff%", (f"{_pw:.1%}" if (_pw := (pz_data["whiff_pct"].mul(pz_data["n_pitches"]).sum() / pz_data["n_pitches"].sum()) if not pz_data.empty and "whiff_pct" in pz_data.columns and pz_data["n_pitches"].sum() > 0 else None) is not None else "—"), "#8aadcc")
