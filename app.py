...
def comp_aggregate_stats(results, pitch_group=None):
    ...
    for r in results:
        ...
        # Store HB as raw pfx_x (hand-agnostic, same as CSV)
...
def hb_to_csv(user_hb, pitcher_hand=None):
    """User enters HB in Statcast pfx_x convention directly.
    pfx_x is catcher-perspective and hand-agnostic:
      negative = toward 3B (arm-side for both RHP and LHP fastballs/sinkers/changeups)
      positive = toward 1B (arm-side for both RHP and LHP sliders/sweepers/curves)
    This means -10 fastball matches BOTH RHP and LHP fastballs in the database.
    No conversion needed — compare pfx_x to pfx_x directly.
    """
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with main_col:
        ...
        hint_html = (
            ...
            "HB: enter Statcast pfx_x — negative = toward 3B (fastball/sinker/CU), positive = toward 1B (slider/sweeper/CB) &nbsp;·&nbsp; iVB: positive = rise"
            "</div>"
        )
        ...
        def pitch_inputs_widget(group, key_prefix, tm_data, show_placeholder=False):
            ...
            with _vc:
                ...
                ivb_s = st.text_input(" ", value=ivb_def_s, key=f"{key_prefix}_ivb",
                                      label_visibility="collapsed")
                st.markdown("<div class='field-label'>HB pfx_x (in)</div>", unsafe_allow_html=True)
    ...
    def build_leaderboard(profiles_hash: int) -> pd.DataFrame:
        ...
        for r in _pdicts_lb:
            ...
            for grp in PITCH_GROUPS:
                ...
                if is_real(hb):
                    hb_disp = float(hb)  # pfx_x direct (hand-agnostic)
                else:
                    hb_disp = None
    ...
    for idx, r in enumerate(results):
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            def display_hb(raw_hb, pitcher_hand=None):
                """Display HB as Statcast pfx_x (same as CSV storage).
                negative = toward 3B, positive = toward 1B.
                Hand-agnostic: -7.6 means fastball movement for both RHP and LHP.
                """
