...
/* Hide ALL clear buttons on text inputs — both browser-native and Streamlit's */
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Row 2: MAX inputs ─────────────────────────────────────────────────────
    def _cb_max(c):
        st.session_state["_lbf"][f"max_{c}"] = pf(st.session_state.get(f"_w_max_{c}", ""))
    def _cb_min(c):
        st.session_state["_lbf"][f"min_{c}"] = pf(st.session_state.get(f"_w_min_{c}", ""))
    def _cb_clear_max(c):
        st.session_state["_lbf"].pop(f"max_{c}", None)
        st.session_state[f"_w_max_{c}"] = ""
    def _cb_clear_min(c):
        st.session_state["_lbf"].pop(f"min_{c}", None)
        st.session_state[f"_w_min_{c}"] = ""
    ...
    for j, col in enumerate(CTRL_COLS):
        with row_max[j+1]:
            ...
            with _in:
                ...
                _def_max = f"{_cur_max:.1f}" if _cur_max is not None else ""
                st.text_input(
                    " ", value=_def_max,
    ...
    for j, col in enumerate(CTRL_COLS):
        with row_min[j+1]:
            ...
            with _in:
                ...
                _def_min = f"{_cur_min:.1f}" if _cur_min is not None else ""
                st.text_input(
                    " ", value=_def_min,
