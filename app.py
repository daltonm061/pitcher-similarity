...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Trigger input: JS writes row index here → Streamlit reruns ──────────
    hm_trigger = st.text_input("lbtrigger", value="",
                                key="_lb_hm_trigger", label_visibility="hidden")
    if hm_trigger and hm_trigger.lstrip("-").isdigit():
        idx_c = int(hm_trigger)
        if idx_c < 0 or idx_c == st.session_state.get("_lb_hm_idx"):
            st.session_state["_lb_hm_idx"] = None
        else:
    ...
    # Hide the trigger input with CSS — collapsed to zero size
    st.markdown(
        "<style>"
        "[data-testid='stTextInput']:has(label p) { "
        "position:fixed !important; top:-9999px !important; "
        "left:-9999px !important; width:0 !important; height:0 !important; "
