...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # st.dataframe(on_select="rerun") returns clicked row index natively.
    # Use pandas Styler for gradient coloring.

    # Build display dataframe from view_display
    ...
    # Flip HB sign for display
    if "HB" in df_show.columns:
        df_show["HB"] = df_show["HB"]  # already flipped in build_leaderboard

    cur_idx = st.session_state.get("_lb_hm_idx", -1)

    # ── Fast vectorized gradient styling using numpy ─────────────────────────
    # Precompute all colors as numpy arrays — no per-cell Python loops
    gradient_cols = [c for c in ["Velo","iVB","HB","VAA","HAA","Stuff+",
                                  "CSW%","Whiff%","xwOBA","Rel Ht","Rel Side",
                                  "Extension","N"] if c in df_show.columns]

    import numpy as np

    def _fast_col_styles(col_name, values, pitch_arr=None, hand_arr=None):
        """Fully vectorized color computation for one column."""
        n = len(values)
        css = np.full(n, "background-color:#141e2e;color:#7aaac0", dtype=object)
        ...
        # ── VAA / HAA: outlier coloring per pitch type ─────────────────────
        if col_name in ("VAA", "HAA") and pitch_arr is not None:
            for i in np.where(valid)[0]:
                pt  = pitch_arr[i]
                vl  = _vaa_haa_league.get(pt, {})
                if not vl:
                    continue
                key = "vaa_mu" if col_name == "VAA" else "haa_mu"
                sdk = "vaa_sd" if col_name == "VAA" else "haa_sd"
                mu, sd = vl.get(key, 0), vl.get(sdk, 1)
                bg, txt = _outlier_color(vals[i], mu, max(sd, 0.001))
                css[i] = f"background-color:{bg};color:{txt}"
            return css

        # ── Rel Ht / Rel Side / Extension: handedness-normalized outlier ───
        if col_name in ("Rel Ht", "Rel Side", "Extension") and hand_arr is not None:
            key_map = {"Rel Ht": "ht", "Rel Side": "side", "Extension": "ext"}
            k = key_map[col_name]
            lb = st.session_state.get("_lb_release_baselines", {})
            if not lb:
                lb = _hand_release_baselines(len(profiles))
                st.session_state["_lb_release_baselines"] = lb
            for i in np.where(valid)[0]:
                hk = f"{hand_arr[i]}_{k}"
                if hk in lb:
                    mu, sd = lb[hk]
        ...
        # ── All other cols: directional gradient using league baseline ─────
        if col_name not in lb_baselines:
            return css
        mu, sd = lb_baselines[col_name]
        if sd <= 0:
            return css
        invert = col_name in ("xwOBA",)
        z = np.clip((vals - mu) / max(sd, 0.001), -2.5, 2.5)
        if invert:
            z = -z
        # Map z (-2.5..2.5) to t (0..1)
        t = (z + 2.5) / 5.0
        # Blue(30,80,128) → grey(120,120,120) → red(220,35,35)
        r = np.where(t < 0.5,
                     (30  + (120-30)  * (t*2)).astype(int),
                     (120 + (220-120) * ((t-0.5)*2)).astype(int))
        g = np.where(t < 0.5,
                     (80  + (120-80)  * (t*2)).astype(int),
                     (120 + (35-120)  * ((t-0.5)*2)).astype(int))
        b = np.where(t < 0.5,
                     (128 + (120-128) * (t*2)).astype(int),
                     (120 + (35-120)  * ((t-0.5)*2)).astype(int))
        lum = (0.299*r + 0.587*g + 0.114*b) / 255
        for i in np.where(valid)[0]:
            txt = "#000000" if lum[i] > 0.45 else "#e8dcc8"
            css[i] = f"background-color:rgb({r[i]},{g[i]},{b[i]});color:{txt}"
        return css

    # Build style DataFrame column by column using vectorized function
    pitch_arr = df_show["Pitch"].values if "Pitch" in df_show.columns else None
    hand_arr  = df_show["Hand"].values  if "Hand"  in df_show.columns else None
    style_df  = pd.DataFrame("", index=df_show.index, columns=df_show.columns)
    for col in gradient_cols:
        style_df[col] = _fast_col_styles(col, df_show[col].values,
                                          pitch_arr=pitch_arr, hand_arr=hand_arr)
    ...
    styled = df_show.style.apply(lambda _: style_df, axis=None).format(fmt, na_rep="—")
    display_data = styled
    ...
    new_idx = rows[0] if rows else -1
    if new_idx != cur_idx:
        st.session_state["_lb_hm_idx"] = None if new_idx < 0 else new_idx
        st.rerun()
