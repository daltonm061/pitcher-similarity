...
def comp_aggregate_stats(results, pitch_group=None):
    ...
    for r in results:
        ...
        # Average in raw Statcast convention — flip to arm-side positive at display time only
        # VAA: negate for display (stored positive in CSV, display convention = negative)
        vaa_display = (-float(vaa)) if _safe(vaa) else None
        for k, v in [("velo",velo),("ivb",ivb),("hb",float(hb) if _safe(hb) else None),("vaa",vaa_display),("haa",float(haa) if _safe(haa) else None),("stuff_plus",sp)]:
    ...
    def render_comp_section(title_label, title_color, groups_to_show):
        ...
        for grp in groups_to_show:
            ...
            def _fv_hb():
                ...
                raw, _ = agg["hb"]
                # Flip to arm-side positive for display (raw is Statcast: RHP arm-side = negative)
                arm = -raw if _dom_hand == "R" else raw
                return f"{arm:+.1f}"

            def _fv_haa():
                if "haa" not in agg: return "—"
                raw, _ = agg["haa"]
                # Flip to arm-side positive for display
                arm = -raw if _dom_hand == "R" else raw
                return f"{arm:+.1f}"
