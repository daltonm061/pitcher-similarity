...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with fc1:
        ...
        # Handle clear-all: set default before multiselect renders
        if st.session_state.pop("_lb_reset_pitches", False):
        ...
        with col_clr:
            ...
            if st.button("✕ All", key="lb_clear_pitches", help="Reset to all pitch types"):
                st.session_state["_lb_reset_pitches"] = True
                st.rerun()
    ...
    # ── Render: CSS-styled sort buttons as header, then HTML tbody ───────────
    # CSS to strip all button chrome and make them look like <th> elements
    st.markdown("""<style>
    .lb-sort-row div[data-testid="stHorizontalBlock"] {
    ...
    .lb-sort-row button:hover {
        background: #162236 !important;
        color: #d4a848 !important;
    }
    .lb-sort-row button[data-active="true"],
    .lb-sort-row button[kind="primary"] {
        background: #162236 !important;
        color: #d4a848 !important;
    ...
    st.markdown("<div class='lb-sort-row'>", unsafe_allow_html=True)
    # Column widths matching the table
    cw = [2.2 if c=="Pitcher" else 0.65 if c in ("Year","Hand") else 1.3 if c=="Pitch" else 0.9 if c=="N" else 1.0 for c in DISPLAY_COLS]
    hdr_cols = st.columns(cw)
    for i_c, col in enumerate(DISPLAY_COLS):
        with hdr_cols[i_c]:
    ...
    table_html = (
        "<div style='overflow-x:auto;border:1px solid #1a2a40;"
