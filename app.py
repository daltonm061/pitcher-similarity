...
def hb_to_csv(user_hb, pitcher_hand="R"):
    """Convert user-entered HB (TrackMan pitcher-perspective convention)
    to Statcast pfx_x (catcher perspective: negative=toward 3B, positive=toward 1B).

    TrackMan reports HB from pitcher perspective:
      RHP: arm-side (right) = positive  → pfx_x = -user_hb
      LHP: arm-side (left)  = negative  → pfx_x = +user_hb

    Examples:
      RHP fastball +14 → pfx_x -14 → CSV -14.9 ✓
      RHP slider   -5  → pfx_x +5  → CSV +4.9  ✓
      LHP fastball -14 → pfx_x -14 → CSV -14.9 ✓
      LHP slider   +5  → pfx_x +5  → CSV +5.0  ✓
    """
    if user_hb is None: return None
    if pitcher_hand == "L":
        return float(user_hb)   # LHP: TrackMan = pfx_x directly
    return -float(user_hb)      # RHP: TrackMan = -pfx_x
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with main_col:
        ...
        hint_html = (
            ...
            "HB: RHP arm-side = positive, LHP arm-side = negative (TrackMan convention) &nbsp;·&nbsp; iVB: positive = rise"
            "</div>"
        )
    ...
    for idx, r in enumerate(results):
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            def display_hb(raw_hb, pitcher_hand="R"):
                """Display HB in TrackMan pitcher-perspective convention.
                CSV pfx_x → TrackMan: RHP = -pfx_x, LHP = +pfx_x
                """
