...
def score_row(user, pitch_inputs, row):
    ...
    for group, metrics in pitch_inputs.items():
        ...
        if has_pitch:
            ...
            # Convert user HB (arm-side positive) to pfx_x for comparison.
            # When no hand specified, always use "R" conversion: -user_hb.
            # This works for both hands because RHP and LHP store same pfx_x for same shape.
            # e.g. -5 slider: hb_to_csv(-5,"R")=+5 → matches RHP +4.9 AND LHP +5.0 ✓
            # When hand IS specified, use that hand's conversion.
            _input_hand = user.get("hand") or "R"
            user_hb = hb_to_csv(user_hb_raw, _input_hand)
...
def run_search_single_pitch(user, velo, ivb, hb_csv, top_n, pitch_type_filter=None):
    ...
    for r in _pdicts:
        ...
        for group in search_groups:
            ...
            # Use input hand (or R default) — finds both hands since pfx_x is symmetric
            _input_hand_sp = user.get("hand") or "R"
            _hb_use = hb_to_csv(hb_csv, _input_hand_sp)
