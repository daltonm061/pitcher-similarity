...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with fc1:
        ...
        with col_clr:
            ...
            if st.button("✕ All", key="lb_clear_pitches", help="Reset to all pitch types"):
                # Delete the key so multiselect reverts to its default on next render
                if "lb_pitches" in st.session_state:
                    del st.session_state["lb_pitches"]
                st.rerun()
    ...
    # ── Sort header row — Streamlit buttons styled as table column headers ──────
    # Style the button row to look exactly like the table header
    st.markdown("""
    <style>
    /* Style the leaderboard sort buttons to look like column headers */
    div[data-testid="stHorizontalBlock"].lb-header-row button {
        background: #0c1420 !important;
        border: none !important;
        border-bottom: 1px solid #1a2a40 !important;
        border-radius: 0 !important;
        color: #7aaac0 !important;
        font-family: 'JetBrains Mono', monospace !important;
    ...
    div[data-testid="stHorizontalBlock"].lb-header-row button:hover {
        background: #162236 !important;
        color: #d4a848 !important;
    }
    div[data-testid="stHorizontalBlock"].lb-header-row button[kind="primary"] {
        background: #162236 !important;
        color: #d4a848 !important;
        font-weight: 700 !important;
        border-bottom: 2px solid #d4a848 !important;
    }
    /* Remove top border radius so header connects flush to table */
    div[data-testid="stHorizontalBlock"].lb-header-row {
        gap: 1px !important;
        background: #1a2a40 !important;
    ...
    # Inject class onto the next horizontal block via a marker
    st.markdown("<div class='lb-header-row' id='lb-hdr'></div>", unsafe_allow_html=True)
    st.markdown("""
    <script>
    // Move lb-header-row class to the next sibling stHorizontalBlock
    (function() {
        var marker = document.getElementById('lb-hdr');
        if (marker) {
            var next = marker.parentElement;
            while (next && !next.getAttribute('data-testid')) next = next.nextElementSibling;
            if (next) next.classList.add('lb-header-row');
        }
    })();
    </script>
    """, unsafe_allow_html=True)

    # Build column width ratios to pass to st.columns
    col_widths = []
    for c in DISPLAY_COLS:
        if c == "Pitcher":       col_widths.append(2.2)
        elif c in ("Year","Hand"): col_widths.append(0.6)
        elif c == "Pitch":       col_widths.append(1.3)
        elif c == "N":           col_widths.append(0.8)
        else:                    col_widths.append(1.0)

    sort_btn_cols = st.columns(col_widths)
    for idx, col in enumerate(DISPLAY_COLS):
        with sort_btn_cols[idx]:
            is_active = st.session_state.get("lb_sort_col") == col
            arrow = (" ↓" if not st.session_state.get("lb_sort_asc", False) else " ↑") if is_active else ""
            btn_type = "primary" if is_active else "secondary"
            if st.button(f"{col}{arrow}", key=f"lbsort_{col}",
    ...
    # Wrap table with margin-top:0 to connect flush to header buttons
    table_html = (
        "<div style='overflow-x:auto;border:1px solid #1a2a40;border-top:none;"
