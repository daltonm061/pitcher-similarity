...
try:
    ...
    league_hh   = (zone_stats.groupby("zone")["hard_hit_pct"].agg(["mean","std"])
                   .rename(columns={"mean":"hh_mu","std":"hh_sd"}))
    zone_league = league_csw.join(league_xwoba).join(league_hh)
    zone_stats_ok = True
except FileNotFoundError:
...
ZONE_GRID = [
    # (zone_id, grid_row, grid_col, row_span, col_span)
    (10,  0, 0, 1, 1),  # top-left outside
    (11,  0, 1, 1, 3),  # top-mid outside (spans inner 3 cols)
    (12,  0, 4, 1, 1),  # top-right outside
    (13,  1, 0, 3, 1),  # mid-left outside (spans inner 3 rows)
    # Inside 3x3
    (1,   1, 1, 1, 1),  (2,  1, 2, 1, 1),  (3,  1, 3, 1, 1),
    (4,   2, 1, 1, 1),  (5,  2, 2, 1, 1),  (6,  2, 3, 1, 1),
    (7,   3, 1, 1, 1),  (8,  3, 2, 1, 1),  (9,  3, 3, 1, 1),
    (14,  1, 4, 3, 1),  # mid-right outside
    (15,  4, 0, 1, 1),  # bot-left outside
    (16,  4, 1, 1, 3),  # bot-mid outside
    (17,  4, 4, 1, 1),  # bot-right outside
]

def _lerp_color(z_score, stat_type):
    """
    Map z-score to a color.
    For CSW%: higher = better for pitcher → red
    For xwOBA: higher = worse for pitcher → red (so invert)
    For HardHit%: higher = worse for pitcher → red (invert)
    """
    # Clamp to [-2.5, 2.5] std range
    z = max(-2.5, min(2.5, z_score if z_score == z_score else 0))
    if stat_type == "csw":
        t = (z + 2.5) / 5.0        # 0=bad(blue), 1=good(red)
    else:
        t = (-z + 2.5) / 5.0       # invert: higher xwOBA/HH = worse
    # Blue → white → Red
    if t < 0.5:
        r = int(40  + (210-40)  * (t*2))
        g = int(90  + (210-90)  * (t*2))
        b = int(200 + (220-200) * (t*2))
    else:
        r = int(220 + (220-220) * ((t-0.5)*2))
        g = int(220 - (220-60)  * ((t-0.5)*2))
        b = int(220 - (220-60)  * ((t-0.5)*2))
    return f"rgb({r},{g},{b})"


def render_zone_heatmap(pitcher_zone_df, stat_col, stat_type, title, fmt=".1%"):
    """
    Render a 5x5 zone grid as an SVG heatmap.
    pitcher_zone_df: rows with zone, stat_col, plus league z-score refs
    Returns SVG string.
    """
    CELL_W, CELL_H = 60, 50
    INNER_W, INNER_H = 48, 42  # inner cell size (3x so 144x126)
    OUTER_W = CELL_W            # outer cell same width
    OUTER_H = CELL_H

    # Total SVG: 5 cols, 5 rows
    # col widths: [OUTER_W, INNER_W, INNER_W, INNER_W, OUTER_W]
    # row heights: [OUTER_H, INNER_H, INNER_H, INNER_H, OUTER_H]
    COL_W = [OUTER_W, INNER_W, INNER_W, INNER_W, OUTER_W]
    ROW_H = [OUTER_H, INNER_H, INNER_H, INNER_H, OUTER_H]
    total_w = sum(COL_W) + 2
    total_h = sum(ROW_H) + 2

    # Pre-index pitcher data by zone
    pdata = {int(row.zone): row for _, row in pitcher_zone_df.iterrows()}

    # col/row pixel offsets
    col_x = [0]
    for w in COL_W[:-1]: col_x.append(col_x[-1] + w)
    row_y = [0]
    for h in ROW_H[:-1]: row_y.append(row_y[-1] + h)

    svg = (
        f"<svg viewBox='0 0 {total_w} {total_h}' "
        f"xmlns='http://www.w3.org/2000/svg' "
        f"style='width:100%;max-width:280px;display:block'>"
        # background
        f"<rect width='{total_w}' height='{total_h}' fill='#080f1a' rx='4'/>"
        # title
        f"<text x='{total_w//2}' y='10' text-anchor='middle' "
        f"font-family='monospace' font-size='8' fill='#3a6a8a' "
        f"text-transform='uppercase' letter-spacing='1'>{title}</text>"
    )

    for (zone_id, gr, gc, rspan, cspan) in ZONE_GRID:
        x = col_x[gc] + 1
        y = row_y[gr] + 14   # offset for title
        w = sum(COL_W[gc:gc+cspan])
        h = sum(ROW_H[gr:gr+rspan])

        is_inside = 1 <= zone_id <= 9

        # Get pitcher's value and league z-score
        row_data = pdata.get(zone_id)
        val = None
        z   = 0.0
        if row_data is not None and stat_col in row_data.index:
            val = row_data[stat_col]
            if val == val:   # not NaN
                mu_col = {"csw_pct":"csw_mu","xwoba_mean":"xw_mu","hard_hit_pct":"hh_mu"}[stat_col]
                sd_col = {"csw_pct":"csw_sd","xwoba_mean":"xw_sd","hard_hit_pct":"hh_sd"}[stat_col]
                if not zone_league.empty and zone_id in zone_league.index:
                    mu = zone_league.loc[zone_id, mu_col]
                    sd = zone_league.loc[zone_id, sd_col]
                    z  = (val - mu) / max(sd, 0.001) if (sd == sd and sd > 0) else 0.0

        fill = _lerp_color(z, stat_type) if val is not None else "#0a1828"
        border = "#c9a84c30" if is_inside else "#0f2030"
        text_col = "#e8dcc8" if val is not None else "#2a5a7a"
        ...
        svg += (
            ...
            f"fill='{fill}' stroke='{border}' stroke-width='1' rx='2'/>"
            f"<text x='{x + w//2}' y='{y + h//2 + 4}' "
            f"text-anchor='middle' font-family='Rajdhani,sans-serif' "
            f"font-size='11' font-weight='700' fill='{text_col}'>{display}</text>"
        )
    ...
    svg += (
        ...
        f"fill='none' stroke='#c9a84c' stroke-width='1.5' rx='0'/>"
    )
    svg += "</svg>"
    return svg
...
def overall_pitcher_zone_data(pitcher_name, year):
    ...
    agg = sub2.groupby("zone").agg(
        ...
        hard_hit_pct = ("hard_hit_pct", "mean"),
    ).reset_index()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for idx, r in enumerate(results):
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            # ── FG Stuff+ (pitcher-season overall from FanGraphs) ─────────
            fg_stuff = row.get("stuff_plus")
            fg_has   = fg_stuff is not None and not (isinstance(fg_stuff, float) and math.isnan(fg_stuff))
            fg_str   = f"{fg_stuff:.0f}" if fg_has else "—"
            fg_col   = stuff_color(fg_stuff) if fg_has else "#3a6a8a"
            fg_lbl   = stuff_grade_label(fg_stuff) if fg_has else "—"

            # ── Release + Stuff+ summary strip ───────────────────────────
            strip = (
                ...
                "<span style='color:#3a6a8a;font-size:9px;text-transform:uppercase;letter-spacing:1px'>FG Stuff+</span>"
                f"<b style='color:{fg_col};font-size:18px'>{fg_str}</b>"
                f"<span style='color:#2a5a7a;font-size:10px'>({fg_lbl}) overall</span>"
                "</span>"
                "</div>"
            )
            ...
            active = [g for g in PITCH_GROUPS if is_real(row.get(f"velo_{g}"))]
            ...
            if not active:
                ...
                for group in active:
                    ...
                    if not pz_data.empty and "n_pitches" in pz_data.columns:
                        ...
                        p_hh     = pz_data["hard_hit_pct"].mean()
                        csw_str  = f"{p_csw:.1%}"
                        xwoba_str= f"{p_xwoba:.3f}" if p_xwoba == p_xwoba else "—"
                        hh_str   = f"{p_hh:.1%}"    if p_hh    == p_hh    else "—"
                    else:
                        csw_str = xwoba_str = hh_str = "—"

                    stuff_badge = (
                        f"<span style='color:#2a5a7a;font-size:9px'>"
                        f"FG S+ <span style='color:{fg_col};font-weight:700'>{fg_str}</span></span>"
                    ) if fg_has else ""
                    ...
                    card_html = (
                        ...
                        f"font-weight:700;color:#8aadcc'>{csw_str}</div></div>"
                        "<div style='text-align:center'>"
                        "<div style='font-family:monospace;font-size:8px;color:#3a6a8a;"
                        "text-transform:uppercase;letter-spacing:1px;margin-bottom:1px'>xwOBA</div>"
                        f"<div style='font-family:Rajdhani,sans-serif;font-size:15px;"
                        f"font-weight:700;color:#8aadcc'>{xwoba_str}</div></div>"
                        "<div style='text-align:center'>"
                        "<div style='font-family:monospace;font-size:8px;color:#3a6a8a;"
                        "text-transform:uppercase;letter-spacing:1px;margin-bottom:1px'>Hard Hit%</div>"
                        f"<div style='font-family:Rajdhani,sans-serif;font-size:15px;"
                        f"font-weight:700;color:#8aadcc'>{hh_str}</div></div>"
                        "</div>"
                    ...
                    if not pz_data.empty:
                        with st.expander(
                            ...
                            with hm2:
                                ...
                                st.markdown(svg, unsafe_allow_html=True)
                            with hm3:
                                svg = render_zone_heatmap(pz_data, "hard_hit_pct", "hh",
                                                          "Hard Hit%", fmt=".1%")
                                st.markdown(svg, unsafe_allow_html=True)
                ...
                if not overall_data.empty:
                    ...
                    with oh2:
                        ...
                        st.markdown(svg, unsafe_allow_html=True)
                    with oh3:
                        svg = render_zone_heatmap(overall_data, "hard_hit_pct", "hh",
                                                  "Hard Hit% — All Pitches", fmt=".1%")
                        st.markdown(svg, unsafe_allow_html=True)
