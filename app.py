    ...
    for idx_r, (_, row) in enumerate(view_display.iterrows()):
        ...
        for col in DISPLAY_COLS:
            ...
            if col == "Pitcher":
                ...
                dot_color = "#d4a848" if is_sel else "#2a5a78"
                icon = "[&gt;]" if is_sel else "[+]"
                link = (
                    f"<a href='?lbr={idx_r}' style='text-decoration:none;margin-right:6px;"
                    f"font-family:monospace;font-size:9px;color:{dot_color};"
                    f"font-weight:700;letter-spacing:0' "
                    f"title='View heatmaps'>{icon}</a>"
                )
                row_cells += (
