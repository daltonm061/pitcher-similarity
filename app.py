    ...
    if not _V5_AVAILABLE:
        ...
        with dm_main:
            ...
            for group in list(dm_added):
                ...
                vc, ic, hc, sc, uc = st.columns([2, 2, 2, 2, 1])
                with vc:
                    st.markdown("<div class='field-label'>Velocity (mph) *</div>", unsafe_allow_html=True)
            ...
            # ── Results ───────────────────────────────────────────────────────
            # Auto re-compute when a suggestion was just applied (flag set
            # by _apply_suggestion callback).
            ...
            if (_do_score or _auto_compute) and dm_added:
                ...
                if pitches_dict:
                    ...
                    if scores:
                        ...
                        # #8 audit: scale legend BEFORE per-pitch results so users
                        # know how to read the scores immediately. v6++ feature
                        # explanations (#11) also live here.
                        _v6_help = ""
                        if any(("stuff_plus_p10" in scores[g] or
                                "stuff_plus_vs_rhb" in scores[g] or
                                "nearest_pitcher" in scores[g] or
                                "ood_warnings" in scores[g])
                        ...
                        for group in dm_added:
                            ...
                            # v6++ Enhanced result row: CI, platoon split,
                            # OOD warnings, NN pitcher
                            _sd = scores[group]
                            _ci_html = ""
                            ...
                            if _ZONE_AVAILABLE and "shape_row" in scores[group]:
                                ...
                                # v6++ #8: per-zone training coverage for opacity dimming
                        ...
                        # ── Arsenal Grade (#3 audit: usage-weighted) ──────────
                        ...
                        try:
                            ...
                            def _best_movement(grp, base_dict, max_delta=4.0):
                                ...
                                Search is capped at ±max_delta inches from the user's
                                current values so results stay attainable."""
                                _bd = _SEARCH_BOUNDS.get(grp, {"ivb": (-15, 25), "hb": (-25, 25)})
                                _pd = base_dict[grp]
                                _velo = _pd.get("velo", 90)
                                _spin = _pd.get("spin_rate")
                                # Centre search on user's current (or imputed) IVB/HB
                                _cur_ivb = _pd.get("ivb") or _V5_MEDIANS["ivb_in"]
                                _cur_hb  = _pd.get("hb")  or _V5_MEDIANS["hb_arm_in"]
                                _iv_lo = max(_bd["ivb"][0], _cur_ivb - max_delta)
                                _iv_hi = min(_bd["ivb"][1], _cur_ivb + max_delta)
                                _hb_lo = max(_bd["hb"][0],  _cur_hb  - max_delta)
                                _hb_hi = min(_bd["hb"][1],  _cur_hb  + max_delta)
                                _best_sp = -1e9
                            ...
                            # #10 audit: lock axis ranges so users can compare across runs
                            _ax.set_xlim(-25, 25)
                            _ax.set_ylim(-25, 25)
                        ...
                        # Each suggestion: (delta, label, detail, action_dict).
                        # action_dict describes how to apply it to session_state
                        # when the user clicks the suggestion button.
                        _suggestions = []
                        ...
                        def _apply_suggestion(action):
                            """Mutate session_state to apply a suggestion and
                            flag auto re-compute on next render."""
                            t = action["type"]
                            if t == "add_pitch":
                                grp = action["group"]
                                ...
                                # number_input expects a float
                                st.session_state[action["key"]] = float(action["value"])
                            # Flag the auto-re-compute (read on next rerun)
