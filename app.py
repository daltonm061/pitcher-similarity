...
def comp_aggregate_stats(results, pitch_group=None):
    ...
    for r in results:
        ...
        # Negate VAA for display: stored positive in CSV, convention is negative = downward
        ...
        if mode == "arsenal":
            ...
            with add_col:
                if remaining:
                    ...
                    if add_choice:
                        st.session_state["_arsenal_pitches"].append(add_choice)
...
"text-transform:uppercase;padding:6px 0 4px 0'>● {group}</div>",
                        unsafe_allow_html=True,
                    )
                with remove_col:
                ...
                with vc:
                    ...
                    velo = st.number_input(" ", min_value=60.0, max_value=105.0,
                                           value=velo_def, step=0.1, format="%.1f",
                                           placeholder="e.g. 93.5", key=f"a_{group}_velo",
                                           label_visibility="collapsed")
                with ic:
                    st.markdown("<div class='field-label'>iVB (in)</div>", unsafe_allow_html=True)
                    ivb_def = float(tm_vals["ivb"]) if tm_vals.get("ivb") is not None else None
                    ivb = st.number_input(" ", min_value=-30.0, max_value=30.0,
                                          value=ivb_def, step=0.1, format="%.1f",
                                          placeholder="e.g. 18.0", key=f"a_{group}_ivb",
                                          label_visibility="collapsed")
                with hc:
                    ...
                    hb = st.number_input(" ", min_value=-30.0, max_value=30.0,
                                         value=hb_def, step=0.1, format="%.1f",
                                         placeholder="e.g. +14", key=f"a_{group}_hb",
                                         label_visibility="collapsed")
            ...
            if _col in ("VAA","HAA","Velo","iVB","HB"):
                for _i in _np.where(_valid)[0]:
                    ...
                    if _col == "VAA":
                        ...
                        # Directional: high iVB = more rise = better → red; low = blue
                        if _pb and "ivb_mu" in _pb:
                            _c = stat_gradient_color(_vals[_i], _pb["ivb_mu"], max(_pb["ivb_sd"],0.001), invert=False)
                            try:
                                _parts = _c[4:-1].split(","); _rv,_gv,_bv = int(_parts[0]),int(_parts[1]),int(_parts[2])
                            except Exception: _rv,_gv,_bv = 20,30,40
                            _lm = (0.299*_rv+0.587*_gv+0.114*_bv)/255
                            _css[_i] = f"background-color:{_c};color:#{'000000' if _lm>0.45 else 'e8dcc8'}"
                    elif _col == "HB":
                        if _pb and "hb_mu" in _pb:
                            # 2-Seam/Sinker: directional — more arm-side run = red, less = blue
                            # RHP: positive values, more positive = more run = red
                            # LHP: negative values, more negative = more run = invert so more negative = red
                            if _pt == "2-Seam/Sinker":
                                _invert_hb = (_hd == "L")  # LHP: less negative = less run = blue
                                _c = stat_gradient_color(_vals[_i], _pb["hb_mu"], max(_pb["hb_sd"],0.001), invert=_invert_hb)
                                try:
                                    _parts = _c[4:-1].split(","); _rv,_gv,_bv = int(_parts[0]),int(_parts[1]),int(_parts[2])
                                except Exception: _rv,_gv,_bv = 20,30,40
                                _lm = (0.299*_rv+0.587*_gv+0.114*_bv)/255
                                ...
                                # All other pitch types: symmetric outlier
                        ...
                        with st.expander(f"📊  {group} zone heatmaps", expanded=False):
                            ...
                            if has_stand_col:
                                ...
                                if not pz_same.empty:
                                    st.markdown("<div style='font-family:JetBrains Mono,monospace;font-size:9px;color:#7aaac0;text-transform:uppercase;letter-spacing:1px;margin:10px 0 4px 0'>vs Same Hand</div>", unsafe_allow_html=True)
                                ...
                                if not pz_opp.empty:
                                    st.markdown("<div style='font-family:JetBrains Mono,monospace;font-size:9px;color:#7aaac0;text-transform:uppercase;letter-spacing:1px;margin:10px 0 4px 0'>vs Opposite Hand</div>", unsafe_allow_html=True)
