...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Table with inline HM buttons ─────────────────────────────────────────
    # CSS targeting buttons by their aria-label (○ or ▸) — only these buttons
    # in the app have these labels so it's safe to target globally
    st.markdown("""<style>
    /* Target button elements directly */
    ...
    /* Collapse ALL wrapper divs around these buttons */
    button[aria-label="○"] ~ div, button[aria-label="▸"] ~ div { display:none !important; }
    div:has(> button[aria-label="○"]),
    div:has(> button[aria-label="▸"]) {
        height: 28px !important;
        min-height: 28px !important;
        max-height: 28px !important;
    ...
    /* Kill the stElementContainer wrapper margin */
    ...
    btn_col, tbl_col = st.columns([0.3, 11])

    with btn_col:
        # Header label aligned with table header
        st.markdown(
            "<div style='height:30px;display:flex;align-items:center;"
            "justify-content:center;background:#0c1420;border-bottom:1px solid #1a2a40;"
            "font-family:monospace;font-size:7px;color:#7aaac0;text-transform:uppercase;"
            "letter-spacing:1px;border-radius:10px 0 0 0'>HM</div>",
            unsafe_allow_html=True,
        )
        for i_b, label in enumerate(row_labels):
            is_active = (i_b == cur_idx)
            if st.button(
                "▸" if is_active else "○",
                key=f"_hmbtn_{i_b}",
                use_container_width=True,
            ):
                st.session_state["_lb_hm_idx"] = None if is_active else i_b
                st.rerun()
