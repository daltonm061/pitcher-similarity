...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Check query param for row click (set by JS in table cell) ───────────
    _qp_lbr = st.query_params.get("lbr")
    if _qp_lbr is not None:
        try:
            st.session_state["_lb_hm_idx"] = int(_qp_lbr)
        except ValueError:
            pass
    ...
    for idx_r, (_, row) in enumerate(view_display.iterrows()):
        ...
        for col in DISPLAY_COLS:
            ...
            if col == "Pitcher":
                ...
                dot_color = "#d4a848" if is_sel else "#3a5a78"
                # 📊 link sets ?lbr=INDEX via JS location — Streamlit reads it on next render
                link = (
                    f"<a href='?lbr={idx_r}' style='text-decoration:none;margin-right:6px;"
                    f"font-size:11px;color:{dot_color};opacity:1' "
                    f"title='View heatmaps'>{'▶' if is_sel else '📊'}</a>"
                )
                row_cells += (
    ...
    # ── Full-width table — no separate button column ─────────────────────────
    table_html = (
