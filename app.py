...
.app-bar-sub {
    font-size: 10px; color: #3d5a78; letter-spacing: 2px; margin-top: 4px;
...
.status-bar {
    ...
    color: #2d4d6d; display: flex; gap: 18px; flex-wrap: wrap; letter-spacing: 0.3px;
}
...
.field-label {
    font-family: 'JetBrains Mono', monospace; font-size: 10px; color: #4a7090;
...
/* ── SLIDER ── */
.stSlider > label { color: #4a7090 !important; font-size: 10px !important;
...
.streamlit-expanderHeader {
    ...
    font-family: 'JetBrains Mono', monospace !important;
    letter-spacing: 0.5px; font-size: 11px !important; font-weight: 500 !important;
    border: 1px solid #162236 !important; border-radius: 10px !important;
    transition: border-color 0.2s !important;
}
.streamlit-expanderHeader:hover { border-color: #1e3250 !important; }
.streamlit-expanderContent {
    background: #0a0e16 !important; border: 1px solid #141e2e !important;
...
.metric-label { color: #4a7090; width: 80px; flex-shrink: 0; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 500; }
.metric-mlb   { width: 70px; text-align: right; font-weight: 600; }
.metric-you   { width: 60px; text-align: right; color: #6a9ab8; }
...
/* ── FILE UPLOADER ── */
.stFileUploader > label { color: #4a7090 !important; font-size: 10px !important;
...
# ── Session state ─────────────────────────────────────────────────────────────
for k, v in [("screen","title"), ("results",None), ("user_snapshot",{}), ("mode","arsenal")]:
...
def comp_zone_data(results, pitch_group=None, stand="all"):
    ...
    for r in results:
        ...
        if not df.empty:
            frames.append(df[["zone", "csw_pct", "xwoba_mean", "n_pitches"]])
    if not frames:
        return pd.DataFrame()
    combined = pd.concat(frames, ignore_index=True)
    # Weighted average CSW, simple mean xwOBA
    combined["csw_weighted"] = combined["csw_pct"] * combined["n_pitches"]
    agg = combined.groupby("zone").agg(
        n_pitches    = ("n_pitches",    "sum"),
        csw_weighted = ("csw_weighted", "sum"),
        xwoba_mean   = ("xwoba_mean",   "mean"),
    ).reset_index()
...
def comp_aggregate_stats(results, pitch_group=None):
    ...
    if not results:
        return {}
    vals = {k: [] for k in ["velo","ivb","hb","vaa","haa","stuff_plus","csw","xwoba"]}
    ...
    for r in results:
        ...
        # CSW and xwOBA from zone_stats (overall per pitcher)
    ...
    for group, metrics in pitch_inputs.items():
        ...
        for metric, sigma in [("ivb",  SIGMA["ivb"]),
            ...
            if not has_pitch:
                ...
                # Penalty softens when pitcher matches most other pitches.
                sim = missing_sim
            else:
...
def run_search_single_pitch(user, velo, ivb, hb_csv, top_n, pitch_type_filter=None):
    ...
    for _, r in profiles.iterrows():
        ...
        # Apply sample confidence
        n = r.get("total_pitches")
        final_score = round(best_pitch_score * sample_confidence(n), 1)
...
if st.session_state.screen == "title":
    ...
    with tc:
        ...
        st.markdown(
            ...
            "color:#4a7090;letter-spacing:6px;text-transform:uppercase;margin-bottom:12px'>"
            ...
            "<div style='font-family:JetBrains Mono,monospace;font-size:10px;color:#3d5a78;"
        ...
        with card_l:
            st.markdown(
                ...
                "Full Arsenal</div>"
                "<div style='font-family:JetBrains Mono,monospace;font-size:10px;color:#5a8aaa;"
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        f"{hdr_label}</div>"
        f"<div style='font-family:JetBrains Mono,monospace;font-size:11px;color:#3d5a78'>{hdr_sub}</div>"
        f"</div>",
        unsafe_allow_html=True,
    )
                ...
                if sp_velo is None and sp_ivb is None and sp_hb_csv is None and not any(v is not None for v in user.values()):
                    ...
                    with st.spinner(spinner_msg):
                        results = run_search_single_pitch(
                            user, sp_velo, sp_ivb, sp_hb_csv, top_n,
                            pitch_type_filter=sp_pitch_type
                        )
                    st.session_state.results = results
                    st.session_state.screen  = "results"
                    st.rerun()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if result_mode == "single":
        # Single-pitch mode: all results have same matched pitch type
        comp_groups = list({r.get("Matched Pitch") for r in results if r.get("Matched Pitch")})
    else:
        # Arsenal mode: show comp panel for each pitch the user entered
        comp_groups = list(pitch_inputs.keys()) if pitch_inputs else []

    def stat_pill(label, val, color, sub=None):
        sub_html = f"<div style='font-family:JetBrains Mono,monospace;font-size:8px;color:{color};opacity:0.7;margin-top:2px'>{sub}</div>" if sub else ""
        return (
            f"<div style='text-align:center;background:{color}18;border:1px solid {color}30;"
            f"border-radius:10px;padding:10px 8px;min-width:72px'>"
            f"<div style='font-family:JetBrains Mono,monospace;font-size:8px;color:{color}99;"
            f"text-transform:uppercase;letter-spacing:1.2px;margin-bottom:4px;font-weight:500'>{label}</div>"
    ...
    def render_comp_section(title_label, title_color, groups_to_show):
        ...
        for grp in groups_to_show:
            ...
            sp_c  = stuff_color(sp_val) if sp_val else "#3a6a8a"
            csw_c = stat_gradient_color(csw_val, 0.28, 0.04) if csw_val else "#3a6a8a"
            xw_c  = stat_gradient_color(xw_val,  0.36, 0.06, invert=True) if xw_val else "#3a6a8a"
            ...
            pills_html = (
                ...
                + stat_pill("HB",   fv("hb","f1"),   "#8aadcc")
                + stat_pill("VAA",  fv("vaa","f1"),  vaa_c)
                + stat_pill("HAA",  fv("haa","f1"),  haa_c)
                + stat_pill("FG S+", fv("stuff_plus","i"), sp_c,
                            stuff_grade_label(sp_val) if sp_val else None)
            ...
            # Heatmaps in columns below pills
            hm_col1, hm_col2, hm_col3, _ = st.columns([2, 2, 2, 1])
            with hm_col1:
                svg = render_zone_heatmap(zdf, "csw_pct", "csw", f"{grp} CSW% (All)", fmt=".1%")
                st.markdown(svg, unsafe_allow_html=True)
            with hm_col2:
                svg = render_zone_heatmap(zdf, "whiff_pct", "whiff", f"{grp} Whiff% (All)", fmt=".1%")
                st.markdown(svg, unsafe_allow_html=True)
            with hm_col3:
                svg = render_zone_heatmap(zdf, "xwoba_mean", "xwoba", f"{grp} xwOBA (All)", fmt=".3f")
                st.markdown(svg, unsafe_allow_html=True)
            ...
            if has_stand_col:
                ...
                st.markdown(
                    "<div style='font-family:JetBrains Mono,monospace;font-size:9px;color:#3d5a78;"
                    "letter-spacing:1px;text-transform:uppercase;margin:8px 0 4px 0'>"
                    "vs Same Hand &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                    "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                    "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                    "vs Opposite Hand</div>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    "<div style='font-family:JetBrains Mono,monospace;font-size:9px;color:#4a7090;"
                    "letter-spacing:1px;text-transform:uppercase;margin:4px 0 2px 0'>"
                    "vs Same Hand</div>", unsafe_allow_html=True)
                hs1, hs2, hs3 = st.columns(3)
                with hs1:
                    svg = render_zone_heatmap(zdf_same, "csw_pct", "csw", "CSW% vs Same", fmt=".1%")
                    st.markdown(svg, unsafe_allow_html=True)
                with hs2:
                    svg = render_zone_heatmap(zdf_same, "whiff_pct", "whiff", "Whiff% vs Same", fmt=".1%")
                    st.markdown(svg, unsafe_allow_html=True)
                with hs3:
                    svg = render_zone_heatmap(zdf_same, "xwoba_mean", "xwoba", "xwOBA vs Same", fmt=".3f")
                    st.markdown(svg, unsafe_allow_html=True)
                st.markdown(
                    "<div style='font-family:JetBrains Mono,monospace;font-size:9px;color:#4a7090;"
                    "letter-spacing:1px;text-transform:uppercase;margin:6px 0 2px 0'>"
                    "vs Opposite Hand</div>", unsafe_allow_html=True)
                ho1, ho2, ho3 = st.columns(3)
                with ho1:
                    svg = render_zone_heatmap(zdf_opp, "csw_pct", "csw", "CSW% vs Opp", fmt=".1%")
                    st.markdown(svg, unsafe_allow_html=True)
                with ho2:
                    svg = render_zone_heatmap(zdf_opp, "whiff_pct", "whiff", "Whiff% vs Opp", fmt=".1%")
                    st.markdown(svg, unsafe_allow_html=True)
                with ho3:
                    svg = render_zone_heatmap(zdf_opp, "xwoba_mean", "xwoba", "xwOBA vs Opp", fmt=".3f")
                    st.markdown(svg, unsafe_allow_html=True)
    ...
    if result_mode == "single" and comp_groups:
        ...
        if not overall_zdf.empty:
            st.markdown(
                ...
                "margin:4px 0 8px 0;border-left:3px solid #c9a84c;padding-left:8px'>"
                "OVERALL ALL-PITCH ZONE AVERAGE</div>",
                unsafe_allow_html=True,
            )
            oz1, oz2, oz3, oz4, oz5 = st.columns(5)
            with oz1:
            ...
            if has_stand_col:
                ...
                with oz3:
                    svg = render_zone_heatmap(ov_same, "csw_pct", "csw", "CSW% vs Same", fmt=".1%")
                    st.markdown(svg, unsafe_allow_html=True)
                with oz4:
                    svg = render_zone_heatmap(ov_same, "xwoba_mean", "xwoba", "xwOBA vs Same", fmt=".3f")
                    st.markdown(svg, unsafe_allow_html=True)
                with oz5:
                    svg = render_zone_heatmap(ov_opp, "csw_pct", "csw", "CSW% vs Opp", fmt=".1%")
                    st.markdown(svg, unsafe_allow_html=True)
                # 6th col for xwOBA vs Opp
                _, ox6 = st.columns([5, 1])
                with ox6:
                    svg = render_zone_heatmap(ov_opp, "xwoba_mean", "xwoba", "xwOBA vs Opp", fmt=".3f")
                    st.markdown(svg, unsafe_allow_html=True)
    ...
    for idx, r in enumerate(results):
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            strip = (
                ...
                "font-family:JetBrains Mono,monospace;font-size:11px;color:#5a8aaa;"
                "background:linear-gradient(165deg,#0e1828,#0c1420);border:1px solid #162236;"
                ...
                "<span style='color:#1e3250;opacity:0.5'>│</span>"
                f"<span>SIM <b style='color:{sc_c};font-size:14px'>{sc:.1f}</b></span>"
                "<span style='margin-left:auto;display:flex;align-items:center;gap:10px'>"
                "<span style='color:#4a7090;font-size:9px;text-transform:uppercase;"
                ...
                "</span>"
                f"<span style='color:#4a7090;font-size:10px'>({fg_overall_lbl})</span>"
                "</span>"
                "</div>"
            )
            ...
            # In single-pitch mode put the matched pitch first, then by usage
            matched = r.get("Matched Pitch") if result_mode == "single" else None
            def pitch_sort_key(g):
                if g == matched:
                    return (0, -pitch_pct(g))   # matched pitch always first
            ...
            if not active:
                st.markdown(
                    "<div style='color:#3d5a78;font-family:JetBrains Mono,monospace;font-size:11px'>"
                ...
                for group in active:
                    ...
                    card_html = (
                        ...
                        f"<span>● {group}</span>"
                        f"<span style='font-size:10px;color:#4a7090;font-weight:400;"
                        ...
                        "<div style='font-family:JetBrains Mono,monospace;font-size:8px;color:#4a7090;"
                        ...
                        f"<div style='font-family:JetBrains Mono,monospace;font-size:8px;color:#3d5a78'>{sp_lbl}</div>"
                        "</div>"
                        "</div>"
                        ...
                        "</div>"
                        f"<span style='font-family:JetBrains Mono,monospace;font-size:9px;"
                        f"color:#233a50'>{n_str} pitches</span>"
                        "</div></div>"
                    )
