...
def score_tjstuff(pitches: dict, rel_height: float = 5.74,
                  rel_side: float = 1.67, extension: float = 6.36,
                  hand: str = "R") -> dict:
    ...
    if _tj_model is None:
        return {}
    # x0 in Statcast: positive = arm-side for RHP, so flip for LHP
    x0 = abs(rel_side) if hand == "R" else -abs(rel_side)
    ...
    p_az    = primary[1].get("az")   or _TJSP_MEDIANS["az"]
    p_ax    = primary[1].get("ax")   or _TJSP_MEDIANS["ax"]
    ...
    for grp, m in pitches.items():
        ...
        ax = m.get("ax") if m.get("ax") is not None else _TJSP_MEDIANS["ax"]
        rows.append({
            "start_speed": velo,
