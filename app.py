...
def comp_zone_data(results, pitch_group=None):
    """
    Build an average zone heatmap across all comp pitchers.
    If pitch_group is None, averages overall (all pitches).
    If pitch_group is a string, filters to that pitch type only.
    Returns a DataFrame matching the format expected by render_zone_heatmap.
    """
...
if st.session_state.screen == "title":
    ...
    with tc:
        ...
        with btn1:
            st.markdown(
                "<div style='background:#0a1828;border:2px solid #c9a84c;border-radius:10px;"
            ...
            st.markdown(
                ...
                "color:#c9a84c;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px'>"
                "Compare Full Arsenal</div>"
                "<div style='font-family:monospace;font-size:10px;color:#3a6a8a;line-height:1.6'>"
                "Match an entire pitch mix to find your<br>closest MLB pitcher comp by arm slot,"
                "<br>velocity, and pitch shape across all pitches."
                "</div>",
                unsafe_allow_html=True,
            )
        ...
        with btn2:
            st.markdown(
                "<div style='background:#0a1828;border:2px solid #2a5a7a;border-radius:10px;"
                "padding:28px 20px;text-align:center;cursor:pointer'>",
                unsafe_allow_html=True,
            )
            st.markdown(
                "<div style='font-size:32px;margin-bottom:8px'>🎯</div>"
                "<div style='font-family:Rajdhani,sans-serif;font-size:18px;font-weight:700;"
                "color:#8aadcc;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px'>"
                "Find Similar Pitches</div>"
                "<div style='font-family:monospace;font-size:10px;color:#3a6a8a;line-height:1.6'>"
                "Enter one pitch's metrics and find the<br>most similar individual pitches"
                "<br>across all MLB pitchers and pitch types."
                "</div>",
                unsafe_allow_html=True,
            )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with main_col:
        ...
        if mode == "arsenal":
            ...
            sp_card_group = sp_pitch_type if sp_pitch_type else "4-Seam"
            sp_col_w, _ = st.columns([3, 6])
            with sp_col_w:
                sp_velo, sp_ivb, sp_hb_csv = pitch_inputs_widget(
                    sp_card_group, "sp", {sp_card_group: first_tm}, show_placeholder=True
                )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def render_comp_section(title_label, title_color, groups_to_show):
        ...
        for grp in groups_to_show:
            ...
            with hm_col1:
                svg = render_zone_heatmap(zdf, "csw_pct", "csw", f"{grp} CSW%", fmt=".1%")
                st.markdown(svg, unsafe_allow_html=True)

            with hm_col2:
                svg = render_zone_heatmap(zdf, "xwoba_mean", "xwoba", f"{grp} xwOBA", fmt=".3f")
                st.markdown(svg, unsafe_allow_html=True)
    ...
    if result_mode == "single" and comp_groups:
        ...
        if not overall_zdf.empty:
            ...
            oz1, oz2, _ = st.columns([2, 2, 3])
            with oz1:
                svg = render_zone_heatmap(overall_zdf, "csw_pct", "csw", "Overall CSW%", fmt=".1%")
                st.markdown(svg, unsafe_allow_html=True)
            with oz2:
                svg = render_zone_heatmap(overall_zdf, "xwoba_mean", "xwoba", "Overall xwOBA", fmt=".3f")
                st.markdown(svg, unsafe_allow_html=True)
