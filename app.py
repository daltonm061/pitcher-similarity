...
def score_row(user, pitch_inputs, row):
    ...
    sim_d  = exp(-0.5 * (delta/σ)²)     ← Gaussian decay per dimension
    score  = exp(Σ w_d * ln(sim_d) / Σ w_d) * 100   ← weighted geometric mean

    Geometric mean means two bad metrics hurt MUCH more than arithmetic mean.
    A perfect match on everything except one σ=2 outlier scores ~30 vs ~77
    with arithmetic mean — more accurately reflecting a bad comp.
    """
    ...
    for key in ("rel_height", "rel_side", "extension"):
        ...
        mv = row.get(key)
        if not is_real(mv):
            sim = 0.4   # missing metric — moderate penalty
        else:
    ...
    for group, metrics in pitch_inputs.items():
        sv = velo_sigma(metrics.get("velo"))   # tighter σ for hard throwers
        ...
        for metric, sigma in [("ivb",  SIGMA["ivb"]),
            ...
            if not is_real(mv):
                sim = 0.4   # pitch type not in MLB profile — moderate penalty
            else:
                sim = gaussian_sim(mv, val, sigma)
    ...
    # Geometric mean → [0,1] range → scale to 0-100
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            if not active:
                ...
                for group in active:
                    ...
                    # ── Look up per-pitch FG Stuff+ from profiles CSV ────────
                    sp_col    = FG_SP_COL.get(group)
                    sp_val    = row.get(sp_col) if sp_col else None
                    sp_has    = sp_val is not None and not (isinstance(sp_val, float) and math.isnan(sp_val))
