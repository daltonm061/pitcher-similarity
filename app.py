...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    @st.cache_data(show_spinner=False)
    def _build_styled(df_hash: int, df_json: str,
                      baselines_hash: int, vaa_hash: int) -> "pd.io.formats.style.Styler":
        ...
        pd.set_option("styler.render.max_elements", len(_df)*len(_df.columns)+1)
        return _df.style.apply(lambda _: _style, axis=None).format(fmt, na_rep="—")

    import json as _json
    _df_hash      = hash(df_show.to_csv())
    _base_hash    = hash(str(sorted(lb_baselines.items())))
    _vaa_hash     = hash(str(sorted(_vaa_haa_league.items())))
    _df_json      = _json.dumps(df_show.to_json(orient="split"))
    styled = _build_styled(_df_hash, _df_json, _base_hash, _vaa_hash)
