...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Add pitch count (N) column from profiles ──────────────────────────────
    if "N" not in lb_df.columns:
        n_rows = []
        for grp in PITCH_GROUPS:
            n_col = f"n_{grp}"
            if n_col in profiles.columns:
                sub = profiles[["player_name","year",n_col]].copy()
                sub.columns = ["player_name","year","N"]
            else:
                # Fallback: estimate from total_pitches × pct if available
                pct_col = f"pct_{grp}"
                if pct_col in profiles.columns:
                    sub = profiles[["player_name","year","total_pitches",pct_col]].copy()
                    sub["N"] = (sub["total_pitches"] * sub[pct_col]).round(0)
                    sub = sub[["player_name","year","N"]]
                else:
                    continue
            sub["Pitch"] = grp
            def _norm_n(nm):
                if "," in str(nm):
                    p = str(nm).split(",",1); return f"{p[1].strip()} {p[0].strip()}"
                return str(nm)
            sub["Pitcher"] = sub["player_name"].apply(_norm_n)
            sub["Year"] = sub["year"].astype(int)
            n_rows.append(sub[["Pitcher","Year","Pitch","N"]])
        if n_rows:
            n_df = pd.concat(n_rows, ignore_index=True)
            lb_df = lb_df.merge(n_df, on=["Pitcher","Year","Pitch"], how="left")

    # ── Filter controls ───────────────────────────────────────────────────────
    METRIC_COLS = ["Velo", "iVB", "HB", "VAA", "HAA", "Stuff+", "CSW%", "xwOBA"]
    if "Whiff%" in lb_df.columns:
        METRIC_COLS.append("Whiff%")
    METRIC_COLS += ["Rel Ht", "Rel Side", "Extension"]
    if "N" in lb_df.columns:
        METRIC_COLS = ["N"] + METRIC_COLS

    fc1, fc2, fc3 = st.columns([2, 1, 1])
    with fc1:
        all_pitches = list(PITCH_GROUPS.keys())
        selected_pitches = st.multiselect("Pitch Types", options=all_pitches,
                                          default=all_pitches, key="lb_pitches")
    with fc2:
        hand_filter = st.selectbox("Hand", ["All", "RHP", "LHP"], key="lb_hand")
    with fc3:
        year_options = ["All"] + sorted(lb_df["Year"].unique().tolist(), reverse=True)
        year_filter = st.selectbox("Season", year_options, key="lb_year")

    # Metric range filters — labeled, 4 metrics per row (min/max pairs)
    filter_cols = [c for c in METRIC_COLS if c in lb_df.columns]
    range_filters = {}

    with st.expander("🔧  Range Filters", expanded=False):
        for i in range(0, len(filter_cols), 4):
            batch = filter_cols[i:i+4]
            row_cols = st.columns(len(batch) * 2)
            for j, metric in enumerate(batch):
                with row_cols[j*2]:
                    st.markdown(f"<div style='font-family:monospace;font-size:9px;color:#7aaac0;"
                                f"text-transform:uppercase;letter-spacing:1px;margin-bottom:2px'>"
                                f"{metric} min</div>", unsafe_allow_html=True)
                    mn = st.number_input(" ", value=None, key=f"lb_min_{metric}",
                                         label_visibility="collapsed", format="%.2f")
                with row_cols[j*2+1]:
                    st.markdown(f"<div style='font-family:monospace;font-size:9px;color:#7aaac0;"
                                f"text-transform:uppercase;letter-spacing:1px;margin-bottom:2px'>"
                                f"{metric} max</div>", unsafe_allow_html=True)
                    mx = st.number_input(" ", value=None, key=f"lb_max_{metric}",
                                         label_visibility="collapsed", format="%.2f")
                if mn is not None or mx is not None:
                    range_filters[metric] = (mn, mx)

    # ── Sort state via session_state (click headers → update sort) ─────────────
    ...
    sort_label_cols = ["Pitcher"] + filter_cols
    st.markdown(
    ...
    sort_btn_cols = st.columns(min(len(sort_label_cols), 8))
    for idx, col_name in enumerate(sort_label_cols[:8]):
        with sort_btn_cols[idx]:
            is_active = st.session_state["lb_sort_col"] == col_name
            arrow = ("↑" if st.session_state["lb_sort_asc"] else "↓") if is_active else ""
            btn_style = "primary" if is_active else "secondary"
            if st.button(f"{col_name} {arrow}", key=f"lb_sortbtn_{col_name}",
                         use_container_width=True, type=btn_style):
                if st.session_state["lb_sort_col"] == col_name:
                    st.session_state["lb_sort_asc"] = not st.session_state["lb_sort_asc"]
                else:
                    st.session_state["lb_sort_col"] = col_name
                    st.session_state["lb_sort_asc"] = False
                st.rerun()

    if len(sort_label_cols) > 8:
        sort_btn_cols2 = st.columns(min(len(sort_label_cols) - 8, 8))
        for idx, col_name in enumerate(sort_label_cols[8:16]):
            with sort_btn_cols2[idx]:
                is_active = st.session_state["lb_sort_col"] == col_name
                arrow = ("↑" if st.session_state["lb_sort_asc"] else "↓") if is_active else ""
                btn_style = "primary" if is_active else "secondary"
                if st.button(f"{col_name} {arrow}", key=f"lb_sortbtn2_{col_name}",
                             use_container_width=True, type=btn_style):
                    if st.session_state["lb_sort_col"] == col_name:
                        st.session_state["lb_sort_asc"] = not st.session_state["lb_sort_asc"]
                    else:
    ...
    for metric, (mn, mx) in range_filters.items():
        if metric in view.columns:
            if mn is not None:
                view = view[view[metric].fillna(-999) >= mn]
            if mx is not None:
                view = view[view[metric].fillna(999) <= mx]

    # Sort FIRST, then take top 500 by the sort column
    if sort_by in view.columns:
        view = view.sort_values(sort_by, ascending=sort_asc, na_position="last")
    elif sort_by == "Pitcher":
        view = view.sort_values("Pitcher", ascending=sort_asc)

    total_rows = len(view)
    display_limit = 500
    view_display = view.head(display_limit)

    st.markdown(
        f"<div style='font-family:JetBrains Mono,monospace;font-size:10px;color:#7aaac0;"
        f"margin:8px 0 4px 0'>{total_rows:,} pitches · showing top {min(display_limit,total_rows):,} by {sort_by}</div>",
        unsafe_allow_html=True,
    )
    ...
    def cell_color(col, val):
        ...
        if c.startswith("rgb"):
            try:
                parts = c[4:-1].split(",")
                r_val, g_val, b_val = int(parts[0]), int(parts[1]), int(parts[2])
            except Exception:
                r_val, g_val, b_val = 20, 30, 40
        else:
            r_val, g_val, b_val = 20, 30, 40
        lum = (0.299*r_val + 0.587*g_val + 0.114*b_val) / 255
        txt = "#000000" if lum > 0.45 else "#e8dcc8"
        return c, txt
    ...
    for col in DISPLAY_COLS:
        is_sort = col == sort_by
        sort_arrow = (" ↓" if not sort_asc else " ↑") if is_sort else ""
        hdr_color = "#d4a848" if is_sort else "#7aaac0"
        header_cells += (
            f"<th style='padding:6px 8px;"
            f"text-align:{'left' if col in ('Pitcher','Pitch') else 'center'};"
            f"font-family:JetBrains Mono,monospace;font-size:9px;color:{hdr_color};"
            f"text-transform:uppercase;letter-spacing:1px;border-bottom:1px solid #1a2a40;"
            f"white-space:nowrap'>{col}{sort_arrow}</th>"
        )

    # ── Heatmap modal state ───────────────────────────────────────────────────
    if "lb_selected" not in st.session_state:
        st.session_state["lb_selected"] = None

    rows_html = ""
    for idx_r, (_, row) in enumerate(view_display.iterrows()):
        row_cells = ""
        pitcher_name = str(row.get("Pitcher",""))
        year_val     = row.get("Year")
        pitch_val    = str(row.get("Pitch",""))

        for col in DISPLAY_COLS:
            val = row.get(col)
            if col == "Pitcher":
                cell_html = f"<td style='padding:5px 8px;font-family:Inter,sans-serif;font-size:11px;font-weight:600;color:#d8cbb4;white-space:nowrap'>{val}</td>"
            elif col == "Year":
                cell_html = f"<td style='padding:5px 8px;text-align:center;font-family:JetBrains Mono,monospace;font-size:10px;color:#8ab0c8'>{int(val) if val else '—'}</td>"
            elif col == "Hand":
                c = "#e8a060" if val == "R" else "#6ab0e8"
                cell_html = f"<td style='padding:5px 8px;text-align:center;font-family:JetBrains Mono,monospace;font-size:10px;color:{c};font-weight:700'>{val}HP</td>"
            elif col == "Pitch":
                pc = PITCH_COLORS.get(str(val), "#8aadcc")
                cell_html = f"<td style='padding:5px 8px;font-family:Inter,sans-serif;font-size:10px;font-weight:700;color:{pc};white-space:nowrap'>● {val}</td>"
            elif col == "N":
                fmt_val = f"{int(val):,}" if val is not None and not (isinstance(val,float) and val!=val) else "—"
                cell_html = f"<td style='padding:5px 8px;text-align:center;font-family:JetBrains Mono,monospace;font-size:10px;color:#8ab0c8'>{fmt_val}</td>"
            else:
    ...
    if total_rows > display_limit:
        rows_html += (
            ...
            f"Showing top {display_limit:,} of {total_rows:,} by {sort_by} — use filters to narrow"
            f"</td></tr>"
        )

    table_html = (
        "<div style='overflow-x:auto;border-radius:10px;border:1px solid #1a2a40'>"
        "<table style='width:100%;border-collapse:separate;border-spacing:0 1px;background:#0a0e18'>"
        f"<thead><tr style='background:#0c1420'>{header_cells}</tr></thead>"
        f"<tbody>{rows_html}</tbody>"
        "</table></div>"
    )
    st.markdown(table_html, unsafe_allow_html=True)

    # ── Pitch heatmap viewer — select from dropdowns below table ──────────────
    st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
    st.markdown(
        "<div style='font-family:JetBrains Mono,monospace;font-size:10px;color:#7aaac0;"
        "text-transform:uppercase;letter-spacing:1px;margin-bottom:8px'>"
        "📊 View Zone Heatmaps for a Pitcher-Season</div>",
        unsafe_allow_html=True,
    )
    hm_col1, hm_col2, hm_col3 = st.columns([3, 2, 2])
    with hm_col1:
        # Build pitcher options from current view
        pitcher_options = sorted(view_display["Pitcher"].unique().tolist())
        sel_pitcher = st.selectbox("Pitcher", ["—"] + pitcher_options, key="lb_hm_pitcher")
    with hm_col2:
        if sel_pitcher != "—":
            year_opts = sorted(view_display[view_display["Pitcher"]==sel_pitcher]["Year"].unique().tolist(), reverse=True)
            sel_year = st.selectbox("Season", year_opts, key="lb_hm_year")
        else:
            st.selectbox("Season", ["—"], key="lb_hm_year")
            sel_year = None
    with hm_col3:
        if sel_pitcher != "—" and sel_year:
            pitch_opts = sorted(view_display[
                (view_display["Pitcher"]==sel_pitcher) &
                (view_display["Year"]==sel_year)
            ]["Pitch"].unique().tolist())
            sel_pitch = st.selectbox("Pitch Type", ["All"] + pitch_opts, key="lb_hm_pitch")
        else:
            st.selectbox("Pitch Type", ["—"], key="lb_hm_pitch")
            sel_pitch = None

    if sel_pitcher != "—" and sel_year and zone_stats_ok:
        # Convert display name back to CSV name format for lookup
        def _to_csv_name(display):
            parts = display.rsplit(" ", 1)
            if len(parts) == 2:
                return f"{parts[1]}, {parts[0]}"
            return display
        csv_name = _to_csv_name(sel_pitcher)

        if sel_pitch and sel_pitch != "All":
            hm_data = pitcher_zone_data(csv_name, sel_year, sel_pitch)
            hm_title = f"{sel_pitcher} {sel_year} — {sel_pitch}"
        else:
            hm_data = overall_pitcher_zone_data(csv_name, sel_year)
            hm_title = f"{sel_pitcher} {sel_year} — All Pitches"

        if not hm_data.empty:
            hm1, hm2, hm3 = st.columns(3)
            with hm1:
                st.markdown(render_zone_heatmap(hm_data, "csw_pct", "csw", f"CSW%", fmt=".1%"), unsafe_allow_html=True)
            with hm2:
                st.markdown(render_zone_heatmap(hm_data, "whiff_pct", "whiff", f"Whiff%", fmt=".1%"), unsafe_allow_html=True)
            with hm3:
                st.markdown(render_zone_heatmap(hm_data, "xwoba_mean", "xwoba", f"xwOBA", fmt=".3f"), unsafe_allow_html=True)
        else:
            st.markdown("<div style='font-family:monospace;font-size:10px;color:#7aaac0;"
                        "padding:12px;background:#0c1420;border-radius:8px;border:1px solid #1a2a40'>"
                        "No zone data for this selection — rebuild pitch_zone_stats.csv</div>",
                        unsafe_allow_html=True)
