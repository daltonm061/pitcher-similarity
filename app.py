...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for col in filter_cols:
        if col in lb_df.columns:
            ...
            if len(vals) > 10:
                # VAA is negated for display but we need positive mean for -val comparison
                if col == "VAA":
    ...
    def _pitch_hand_baselines(profiles_hash: int):
        ...
        All values in display convention:
          HB  — arm-side positive (RHP flipped)
          VAA — stored positive (raw), cell_color does -val comparison
          HAA — as stored
        """
        ...
        for grp in PITCH_GROUPS:
            for hand_f, key_suffix in [("R", "_R"), ("L", "_L"), (None, "")]:
                ...
                for metric, col_name in [
                    ...
                    if metric == "hb" and hand_f == "R":
                        vals = -vals   # arm-side positive for display
                    if len(vals) > 10:
    ...
    def cell_color(col, val, pitch_type=None, hand=None):
        ...
        # ── VAA ───────────────────────────────────────────────────────────────
        # Display value is negated; baselines store raw positive → compare -val
        if col == "VAA":
        ...
        # ── Velo: pitch+hand (fastball ~93, curve ~79, etc.) ─────────────────
