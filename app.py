...
.app-bar-sub {
    font-size: 10px; color: #6a90a8; letter-spacing: 2px; margin-top: 4px;
...
.status-bar {
    ...
    color: #5a8098; display: flex; gap: 18px; flex-wrap: wrap; letter-spacing: 0.3px;
}
...
def run_search_single_pitch(user, velo, ivb, hb_csv, top_n, pitch_type_filter=None):
    ...
    for _, r in profiles.iterrows():
        ...
        for group in search_groups:
            ...
            # This prevents a sinker showing up when searching a 4-seam
            if ivb is not None and is_real(mv_ivb):
                if abs(float(mv_ivb) - float(ivb)) > 5.0:
                    continue
            if hb_csv is not None and is_real(mv_hb):
                if abs(float(mv_hb) - float(hb_csv)) > 6.0:
                    continue
            # Score this pitch
            p_log = log_sum
            p_w   = total_w
        ...
        if multi_pitch_mode:
            ...
            for group in search_groups:
                ...
                if not is_real(r.get(f"velo_{group}")):
                    continue
                # Apply same hard shape cutoff as the best-pitch loop above
                if ivb is not None and is_real(mv_ivb_g):
                    if abs(float(mv_ivb_g) - float(ivb)) > 5.0:
                        continue
                if hb_csv is not None and is_real(mv_hb_g):
                    if abs(float(mv_hb_g) - float(hb_csv)) > 6.0:
                        continue
