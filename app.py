...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    import re as _re
    tr_parts = _re.split(r'(<tr[^>]*>)', rows_html)
    rows_with_hm = ""
    i_r = 0
    for part in tr_parts:
        if _re.match(r'<tr', part):
            rows_with_hm += part
    ...
    # Use raw Streamlit component protocol — no CDN needed
    # Must send componentReady first, then setComponentValue on click
    component_html = """<script>
// Announce ready to Streamlit parent
...
""" + full_table
    ...
    clicked = st.components.v1.html(
        component_html,
        height=hdr_h + row_h * len(row_labels) + 8,
        scrolling=False,
    )
    if clicked is not None:
        try:
            idx_c = int(float(str(clicked)))
