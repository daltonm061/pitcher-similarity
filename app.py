...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Render entire table as single st.components.v1.html ─────────────────
    # This puts everything in one iframe so JS can update session state
    # via window.parent.location without cross-origin issues.
    # The HM column is a real <td> inside the table — no wrapper divs.

    # Read click from query params (set by JS in component)
    _qp = st.query_params.get("lbr")
    if _qp is not None:
        try:
            _ni = int(_qp)
            st.session_state["_lb_hm_idx"] = None if _ni == st.session_state.get("_lb_hm_idx") else _ni
        except Exception:
            pass
        st.query_params.clear()
        st.rerun()
    ...
    # Build HM header cell
    hm_th = (
        "<th style='padding:7px 6px;text-align:center;background:#0c1420;"
        "font-family:JetBrains Mono,monospace;font-size:9px;color:#7aaac0;"
        "text-transform:uppercase;letter-spacing:1px;border-bottom:1px solid #1a2a40;"
        "white-space:nowrap;width:28px'>HM</th>"
    )

    # Build HM td cells parallel to rows_html
    hm_tds = []
    for i_r in range(len(row_labels)):
        is_sel    = (i_r == cur_idx)
        icon      = "&#9654;" if is_sel else "&#9711;"
        bg        = "#1a2a3a" if is_sel else "#0a0e18"
        color     = "#d4a848" if is_sel else "#2a5a78"
        bdr       = "border-left:2px solid #d4a848;" if is_sel else ""
        td_style  = f"width:28px;padding:0;text-align:center;{bdr}border-bottom:1px solid #0f1820;background:{bg}"
        btn_style = f"width:100%;height:100%;min-height:28px;background:none;border:none;cursor:pointer;color:{color};font-size:11px;padding:0"
        nav       = f"window.parent.location.href=window.parent.location.pathname+'?lbr={i_r}'"
        hm_tds.append(
            f"<td style='{td_style}'>"
            f"<button onclick='{nav}' style='{btn_style}'>{icon}</button></td>"
        )

    # Inject HM td into each row by splitting on <tr and reinserting
    import re
    tr_parts = re.split(r'(<tr[^>]*>)', rows_html)
    rows_with_hm = ""
    i_r = 0
    for part in tr_parts:
        if re.match(r'<tr', part):
            rows_with_hm += part
        elif part.startswith("<td") and i_r < len(hm_tds):
            rows_with_hm += hm_tds[i_r] + part
            i_r += 1
        else:
            rows_with_hm += part
    ...
    table_html = (
        ...
        f"<thead><tr>{hm_th}{header_cells}</tr></thead>"
        f"<tbody>{rows_with_hm}</tbody></table></div>"
    )
