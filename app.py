...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Column widths: label col + one per metric
    CTRL_COLS  = filter_cols   # metrics only (not Pitcher/Year/Hand/Pitch)
    label_w    = 0.7
    metric_w   = 1.0
    col_widths = [label_w] + [metric_w] * len(CTRL_COLS)

    # ── Row 1: SORT buttons ───────────────────────────────────────────────────
    row1 = st.columns(col_widths)
    with row1[0]:
        st.markdown(
            "<div style='font-family:monospace;font-size:9px;color:#7aaac0;"
            "text-transform:uppercase;letter-spacing:1px;padding:6px 0;"
            "text-align:right;padding-right:8px'>SORT</div>",
            unsafe_allow_html=True,
        )
    for j, col in enumerate(CTRL_COLS):
        with row1[j+1]:
            is_active = (col == sort_by)
            arrow = (" ↓" if not sort_asc else " ↑") if is_active else ""
            if st.button(
                f"{col}{arrow}",
                key=f"lbsort_{col}",
    ...
    st.markdown(
        ...
        f"{total_rows:,} pitches · top {min(500,total_rows):,} by "
        f"{sort_by} {'↑' if sort_asc else '↓'}</div>",
        unsafe_allow_html=True,
    )
    ...
    for idx_r, (_, row) in enumerate(view_display.iterrows()):
        row_cells = ""
        pitch_type = row.get("Pitch")  # pass to cell_color for VAA/HAA
        row_hand   = row.get("Hand")   # pass to cell_color for Rel Ht/Side
        for col in DISPLAY_COLS:
            val = row.get(col)
            if col == "Pitcher":
                is_sel = (st.session_state.get("_lb_hm_idx") == idx_r)
                row_cells += (
                    f"<td style='padding:5px 8px;font-family:Inter,sans-serif;"
                    f"font-size:11px;font-weight:600;color:#d8cbb4;white-space:nowrap'>"
                    f"{val}</td>"
                )
            elif col == "Year":
                ...
                bg, txt = cell_color(col, val, pitch_type=pitch_type, hand=row_hand)
                fmt_val = "—"
    ...
    if total_rows > 500:
        rows_html += (
            ...
            f"Top 500 of {total_rows:,} by {sort_by} — use filters to narrow</td></tr>"
        )
    ...
    for col in DISPLAY_COLS:
        ...
        fw    = "700" if is_sort else "500"
        header_cells += (
            f"<th style='padding:7px 8px;text-align:{align};"
            f"font-family:JetBrains Mono,monospace;font-size:9px;color:{color};"
            f"text-transform:uppercase;letter-spacing:1px;"
            f"background:{bg};border-bottom:{bdr};"
            f"white-space:nowrap;font-weight:{fw}'>{col}{arrow}</th>"
        )
    ...
    # ── Button column + table using components for zero-gap buttons ──────────
    # Build compact HTML button panel via st.components.v1.html
    # This renders in its own iframe and can communicate via Streamlit's
    # component protocol

    # ── Hidden text input that JS writes to, triggering Streamlit rerun ─────
    # CSS hides it completely; JS in the component sets its value and fires
    # the native input event so Streamlit detects the change
    st.markdown("""
    <style>
    div[data-testid="stTextInput"].lb-hidden-input {
    ...
    st.markdown("<div class='lb-hidden-input'>", unsafe_allow_html=True)
    hm_trigger = st.text_input(" ", value="", key="_lb_hm_trigger",
                                label_visibility="collapsed")
    st.markdown("</div>", unsafe_allow_html=True)

    # React to trigger value
    if hm_trigger and hm_trigger.lstrip("-").isdigit():
        idx_c = int(hm_trigger)
        if idx_c < 0:
            st.session_state["_lb_hm_idx"] = None
        elif idx_c != st.session_state.get("_lb_hm_idx"):
            st.session_state["_lb_hm_idx"] = idx_c
        # Clear trigger
        del st.session_state["_lb_hm_trigger"]
        st.rerun()
    ...
    btn_rows = ""
    for i_b in range(len(row_labels)):
        cls  = "active" if i_b == cur_idx else ""
        icon = "&#9654;" if i_b == cur_idx else "&#9711;"
        # JS: find the hidden text input, set value, fire input + change events
        js = (
            "var el=window.parent.document.querySelector('[data-testid=stTextInput]"
            ".lb-hidden-input input');"
            "if(!el)el=window.parent.document.querySelectorAll"
            "('[data-testid=stTextInput] input')[0];"
            "if(el){el.value='" + str(i_b) + "';"
            "el.dispatchEvent(new Event('input',{bubbles:true}));"
            "el.dispatchEvent(new Event('change',{bubbles:true}));}"
        )
        btn_rows += '<button class="' + cls + '" onclick="' + js + '">' + icon + '</button>\n'


    btns_html = (
        "<style>"
        "body{margin:0;padding:0;background:#0a0e18;overflow:hidden}"
        f".hdr{{height:{HDR_H}px;background:#0c1420;border-bottom:1px solid #1a2a40;"
        "display:flex;align-items:center;justify-content:center;"
        "font-family:monospace;font-size:8px;color:#7aaac0;"
        "text-transform:uppercase;letter-spacing:1px}"
        f"button{{display:block;width:100%;height:{ROW_H}px;"
        "background:#0a0e18;border:none;border-bottom:1px solid #0f1820;"
        "color:#2a5a78;font-family:monospace;font-size:11px;"
        "font-weight:700;cursor:pointer;padding:0;margin:0}"
        "button:hover{background:#162236;color:#d4a848}"
        "button.active{background:#162236;color:#d4a848;border-left:2px solid #d4a848}"
        "</style>"
        "<div class='hdr'>HM</div>"
        + btn_rows
    )

    total_h = HDR_H + ROW_H * len(row_labels) + 4
    btn_col, tbl_col = st.columns([0.35, 11])

    with btn_col:
        st.components.v1.html(btns_html, height=total_h, scrolling=False)

    with tbl_col:
        table_html = (
            "<div style='overflow-x:auto;border-radius:0 10px 10px 0;"
            "border:1px solid #1a2a40;border-left:none'>"
            "<table style='width:100%;border-collapse:collapse;background:#0a0e18'>"
