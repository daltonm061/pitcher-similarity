...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Table with HM buttons using Streamlit component API ─────────────────
    # Load Streamlit's component communication library inside the iframe
    # so Streamlit.setComponentValue() works and triggers a Python rerun.
    ...
    for i_r in range(len(row_labels)):
        ...
        hm_tds.append(
            f"<td style='{td_style}'>"
            f"<button onclick='pick({i_r})' style='{btn_style}'>{icon}</button></td>"
        )
    ...
    # Wrap in a component that uses Streamlit's JS API for bidirectional comms
    component_html = """
...
""" + full_table
    ...
    clicked = st.components.v1.html(component_html,
                                     height=35 + 29 * len(row_labels) + 20,
                                     scrolling=False)
    if clicked is not None:
        try:
            idx_c = int(clicked)
            st.session_state["_lb_hm_idx"] = None if idx_c == cur_idx else idx_c
            st.rerun()
        except Exception:
            pass
