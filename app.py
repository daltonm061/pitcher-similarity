...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for idx, r in enumerate(results):
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            # ── Release + FG Stuff+ summary strip ────────────────────────
            ...
            strip = (
                ...
                f"FG STUFF+ <b style='color:{fg_col};font-size:14px'>{fg_str}</b>"
                f"<span style='color:#2a5a7a;font-size:10px'> ({fg_lbl})</span></span>"
                "</div>"
            )
            ...
            if not active:
                ...
                for row_start in range(0, len(active), 3):
                    chunk = active[row_start:row_start + 3]
                    cols  = st.columns(len(chunk))
                    for col, group in zip(cols, chunk):
                        color   = PITCH_COLORS[group]
                        ...
                        hb_s   = f"{mv_hb:.1f}\""   if is_real(mv_hb)   else "—"
                        ...
                        def sub(val, color_hex="#5a8aaa"):
                            if val:
                                return (
                                    f"<div style='font-family:monospace;font-size:9px;"
                                    f"color:{color_hex};margin-top:1px'>{val}</div>"
                                )
                            return ""

                        card = (
                            f"<div style='background:#0a1828;border:1px solid #0f2030;"
                            f"border-top:2px solid {color};border-radius:6px;padding:12px 14px'>"
                            # pitch label
                            ...
                            f"<span>VAA {vaa_s}</span>"
                            f"<span>HAA {haa_s}</span>"
                            "</div>"
                            "</div>"
                        )
                        with col:
