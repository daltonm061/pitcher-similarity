...
/* Hide label — Streamlit 1.55 wraps label inside stNumberInputContainer */
.stNumberInput label,
.stNumberInput > label,
[data-testid="stNumberInputContainer"] label,
[data-testid="stNumberInputContainer"] > label,
[data-testid="stNumberInput"] label,
div[data-testid="stNumberInputContainer"] > div > label,
.stNumberInput [data-testid="stWidgetLabel"],
[data-testid="stWidgetLabel"] { display: none !important; height: 0 !important; overflow: hidden !important; margin: 0 !important; padding: 0 !important; }
...
# ── Zone grid layout (5×5 grid, catcher's perspective) ──────────────────────
# Inside 9 zones (1-9) occupy the center 3×3.
# Outside 16 zones (11-26) surround them:
#   Top row:    11,12,13,14,15  (5 cells across top)
#   Mid rows:   16,17 left/right flanks × 3 rows = 6 cells
#   Bottom row: 21,22,23,24,25  (5 cells across bottom)
# Grid coordinates: (zone_id, grid_row, grid_col) — 5×5 grid
#   grid_row 0=top-out, 1-3=inner rows, 4=bot-out
#   grid_col 0=left-out, 1-3=inner cols, 4=right-out
...
OUTSIDE_ZONES = [
    ...
    # mid-left (col 0) rows 1-3
    (16,1,0),(17,2,0),(18,3,0),
    # mid-right (col 4) rows 1-3
...
def render_zone_heatmap(pitcher_zone_df, stat_col, stat_type, title, fmt=".1%"):
    ...
    mu_col = {"csw_pct": "csw_mu", "xwoba_mean": "xw_mu"}.get(stat_col)
    sd_col = {"csw_pct": "csw_sd", "xwoba_mean": "xw_sd"}.get(stat_col)
    ...
    for (zone_id, gr, gc) in ALL_ZONES:
        ...
        if val is not None:
            ...
            fill = "#080f1a"   # outer empty cells slightly darker

        txt_fill = "#000000" if val is not None else ("#1a3a5a" if is_inside else "#0f2535")
        ...
        svg += (
            ...
            f"fill='{fill}' stroke='#0f2030' stroke-width='0.5'/>"
...
if st.session_state.screen == "title":
    ...
    with tc:
        ...
        st.markdown(
            ...
            "color:#e8dcc8;letter-spacing:4px;text-transform:uppercase;line-height:1.1'>"
            "MLB PITCHER</div>"
            "<div style='font-family:Rajdhani,sans-serif;font-size:48px;font-weight:700;"
            "color:#c9a84c;letter-spacing:4px;text-transform:uppercase;line-height:1.1'>"
            "SIMILARITY FINDER</div>"
            "<div style='font-family:monospace;font-size:11px;color:#2a5a7a;"
            "margin-top:14px;letter-spacing:1px'>"
            "Statcast 2017–2024 &nbsp;·&nbsp; Gaussian similarity scoring &nbsp;·&nbsp; Factor-matched comps"
            "</div></div>",
            unsafe_allow_html=True,
        )
        ...
        with card_l:
            st.markdown(
                "<div style='background:#0a1828;border:2px solid #c9a84c;border-radius:12px;"
                "padding:32px 24px 20px 24px;text-align:center;margin-bottom:4px'>"
                "<div style='font-size:36px;margin-bottom:12px'>⚾</div>"
                "<div style='font-family:Rajdhani,sans-serif;font-size:20px;font-weight:700;"
                "color:#c9a84c;letter-spacing:2px;text-transform:uppercase;margin-bottom:10px'>"
                "Compare Full Arsenal</div>"
                "<div style='font-family:monospace;font-size:10px;color:#3a6a8a;line-height:1.7'>"
        ...
        with card_r:
            st.markdown(
                "<div style='background:#0a1828;border:2px solid #2a5a7a;border-radius:12px;"
                "padding:32px 24px 20px 24px;text-align:center;margin-bottom:4px'>"
                "<div style='font-size:36px;margin-bottom:12px'>🎯</div>"
                "<div style='font-family:Rajdhani,sans-serif;font-size:20px;font-weight:700;"
                "color:#8aadcc;letter-spacing:2px;text-transform:uppercase;margin-bottom:10px'>"
                "Find Similar Pitches</div>"
                "<div style='font-family:monospace;font-size:10px;color:#3a6a8a;line-height:1.7'>"
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        f"color:#c9a84c;letter-spacing:2px;text-transform:uppercase;margin-bottom:6px'>"
        f"{hdr_label}</div>"
        f"<div style='font-family:monospace;font-size:11px;color:#2a5a7a'>{hdr_sub}</div>"
        f"</div>",
        unsafe_allow_html=True,
    )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def stat_pill(label, val, color, sub=None):
        sub_html = f"<div style='font-family:monospace;font-size:8px;color:{color};opacity:0.7;margin-top:1px'>{sub}</div>" if sub else ""
        # Use black text on colored pills for readability
        return (
            f"<div style='text-align:center;background:{color};border:1px solid {color};"
            "border-radius:6px;padding:8px 6px;min-width:70px'>"
            f"<div style='font-family:monospace;font-size:8px;color:#00000099;"
    ...
    def render_comp_section(title_label, title_color, groups_to_show):
        ...
        st.markdown(
            f"<div style='font-family:Rajdhani,sans-serif;font-size:13px;font-weight:700;"
        ...
        for grp in groups_to_show:
            ...
            # Stats row + heatmaps side by side
            stats_col, hm_col1, hm_col2 = st.columns([3, 2, 2])
            ...
            with stats_col:
                pills_html = (
                    ...
                    + stat_pill("VAA",  fv("vaa","f1"),  "#6aacaa")
                    + stat_pill("HAA",  fv("haa","f1"),  "#6aacaa")
            ...
            if not active:
                ...
                for group in active:
                    ...
                    card_html = (
                        ...
                        f"<span>● {group}</span>"
                        f"<span style='font-size:10px;color:#3a6a8a;font-weight:400;"
                        ...
                        f"<span style='color:#1e3a50'>{n_str} pitches</span>"
                        "</div></div>"
                    )
