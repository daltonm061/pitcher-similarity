...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Default initial sort: Velo descending
    if "Velo" in view.columns:
        view = view.sort_values("Velo", ascending=False, na_position="last")

    total_rows   = len(view)
    view_display = view  # all rows — native dataframe sort works on full dataset
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # CONTROL PANEL — 3 rows above the table:
    #   Row 1 (SORT):  label col | one button per metric — click = sort desc,
    #                  click again = sort asc
    #   Row 2 (MAX):   label col | one number_input per metric
    #   Row 3 (MIN):   label col | one number_input per metric
    # ══════════════════════════════════════════════════════════════════════════

    st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)

    # CTRL_COLS for MAX/MIN filter rows
    ...
    # ── Row 1: MAX inputs ────────────────────────────────────────────────────
    # (Sorting is now handled by clicking column headers in the dataframe below)
    # Use a shadow dict to store filter values — number_input writes here,
    # filter reads from here. This survives sort reruns without resetting.
    row2 = st.columns(col_widths)
    with row2[0]:
        st.markdown(
            "<div style='font-family:monospace;font-size:9px;color:#7aaac0;"
            "text-transform:uppercase;letter-spacing:1px;padding:4px 0;"
            "text-align:right;padding-right:8px'>MAX</div>",
            unsafe_allow_html=True,
        )
    # Use on_change to write into stable _lbf dict so values survive reruns
    ...
    for j, col in enumerate(CTRL_COLS):
        with row2[j+1]:
            st.number_input(
                ...
                key=f"_w_max_{col}",
                on_change=_cb_max, args=(col,),
                label_visibility="collapsed", format="%.1f", step=0.1,
            )

    # ── Row 3: MIN inputs ────────────────────────────────────────────────────
    row3 = st.columns(col_widths)
    with row3[0]:
        st.markdown(
            "<div style='font-family:monospace;font-size:9px;color:#7aaac0;"
            "text-transform:uppercase;letter-spacing:1px;padding:4px 0;"
            "text-align:right;padding-right:8px'>MIN</div>",
            unsafe_allow_html=True,
        )
    for j, col in enumerate(CTRL_COLS):
        with row3[j+1]:
            st.number_input(
                ...
                key=f"_w_min_{col}",
                on_change=_cb_min, args=(col,),
                label_visibility="collapsed", format="%.1f", step=0.1,
            )

    st.markdown(
        f"<div style='font-family:JetBrains Mono,monospace;font-size:10px;"
        f"color:#7aaac0;margin:6px 0 4px 0'>"
        f"{total_rows:,} pitches — click any column header to sort</div>",
        unsafe_allow_html=True,
    )

    # Build row labels here so they're available for both heatmap viewer and buttons
    row_labels = [f"{r['Pitcher']}  {r['Year']}  ●{r['Pitch']}"
                  for _, r in view_display.iterrows()]

    # Initialize selected row
    # Index-based selection (set by clicking 📊 link in table)
    _hm_idx = st.session_state.get("_lb_hm_idx")
    if _hm_idx is not None and (_hm_idx < 0 or _hm_idx >= len(view_display)):
        _hm_idx = None  # out of range after filter change
    ...
    if _hm_idx is not None:
        ...
        if not zone_stats_ok or zone_stats.empty:
            st.markdown(
                ...
                "run <code>build_profiles.py</code> locally and commit the file.</div>",
                unsafe_allow_html=True,
            )
        else:
            # Convert "First Last" → "Last, First" for zone_stats lookup
            def _csv_name(dn):
                parts = dn.rsplit(" ", 1)
            ...
            if not hm_data.empty:
                ...
                # Same/opp splits if stand column exists
                if "stand" in zone_stats.columns:
                ...
                st.markdown(
                    ...
                    f"No zone data found for {sel_pitcher} {sel_year} {sel_pitch}. "
                    f"Try rebuilding pitch_zone_stats.csv.gz.</div>",
                    unsafe_allow_html=True,
                )

    # ── Sort control + HTML table with row selection ────────────────────────
    # Compact sort: one selectbox + direction button
    # HTML table for display (fast, gradient colored)
    # st.dataframe (hidden, single row) just for row click detection
    ...
    _s1, _s2, _s3 = st.columns([2, 0.5, 8])
    with _s1:
        _sort_opts = ["Pitcher"] + filter_cols
        _cur_sort  = st.session_state["lb_sort_col"]
        _sort_idx  = _sort_opts.index(_cur_sort) if _cur_sort in _sort_opts else 1
        _new_sort  = st.selectbox("Sort", _sort_opts, index=_sort_idx,
                                   key="_lb_sort_sel", label_visibility="collapsed")
        if _new_sort != _cur_sort:
            st.session_state["lb_sort_col"] = _new_sort
            st.session_state["_lb_hm_idx"] = None
            st.rerun()
    with _s2:
        _asc = st.session_state["lb_sort_asc"]
        if st.button("↑" if _asc else "↓", key="_lb_dir"):
            st.session_state["lb_sort_asc"] = not _asc
            st.rerun()
    ...
    elif _sort_by == "Pitcher":
        view = view.sort_values("Pitcher", ascending=_sort_asc)
    ...
    for idx_r, (_, row) in enumerate(view_display.iterrows()):
        is_sel  = (idx_r == cur_idx)
        row_bg  = "background:#162236" if is_sel else ""
        cells   = ""
        pt = row.get("Pitch")
        hd = row.get("Hand")
        for col in disp_cols:
            val = row.get(col)
            if col == "Pitcher":
                cells += f"<td style='padding:5px 8px;font-family:Inter,sans-serif;font-size:11px;font-weight:600;color:#d8cbb4;white-space:nowrap'>{val}</td>"
            elif col == "Year":
                cells += f"<td style='padding:5px 6px;text-align:center;font-family:monospace;font-size:10px;color:#8ab0c8'>{int(val) if val else '—'}</td>"
            elif col == "Hand":
                c = "#e8a060" if val=="R" else "#6ab0e8"
                cells += f"<td style='padding:5px 6px;text-align:center;font-family:monospace;font-size:10px;color:{c};font-weight:700'>{val}HP</td>"
            elif col == "Pitch":
                pc = PITCH_COLORS.get(str(val), "#8aadcc")
                cells += f"<td style='padding:5px 6px;font-family:Inter,sans-serif;font-size:10px;font-weight:700;color:{pc};white-space:nowrap'>● {val}</td>"
            elif col == "N":
                fmt_n = f"{int(val):,}" if val is not None and not (isinstance(val,float) and val!=val) else "—"
                cells += f"<td style='padding:5px 6px;text-align:center;font-family:monospace;font-size:10px;color:#8ab0c8'>{fmt_n}</td>"
            else:
                bg, txt = cell_color(col, val, pitch_type=pt, hand=hd)
                if val is None or (isinstance(val,float) and val!=val):
                    fmt_v = "—"
                elif col in ("CSW%","Whiff%"):  fmt_v = f"{val:.1f}%"
                elif col == "xwOBA":            fmt_v = f"{val:.3f}"
                elif col in ("Stuff+","N"):     fmt_v = f"{int(val)}"
                elif col in ("HB","VAA","HAA"): fmt_v = f"{val:+.1f}"
                else:                           fmt_v = f"{val:.1f}"
                cells += (f"<td style='padding:4px 6px;text-align:center;background:{bg};"
                          f"color:{txt};font-family:monospace;font-size:11px;font-weight:600;"
                          f"border-radius:3px'>{fmt_v}</td>")
        rows_html += f"<tr style='border-bottom:1px solid #0f1820;{row_bg}'>{cells}</tr>"

    # Header
    header_cells = ""
    for col in disp_cols:
        is_sort = (col == _sort_by)
        color = "#d4a848" if is_sort else "#7aaac0"
        bg    = "#162236" if is_sort else "#0c1420"
        bdr   = "2px solid #d4a848" if is_sort else "1px solid #1a2a40"
        align = "left" if col in ("Pitcher","Pitch") else "center"
        arrow = (" ↓" if not _sort_asc else " ↑") if is_sort else ""
        header_cells += (
            f"<th style='padding:7px 8px;text-align:{align};background:{bg};"
            f"font-family:monospace;font-size:9px;color:{color};"
            f"text-transform:uppercase;letter-spacing:1px;border-bottom:{bdr};"
            f"white-space:nowrap;font-weight:700'>{col}{arrow}</th>"
        )

    st.markdown(
        f"<div style='overflow-x:auto;border-radius:10px;border:1px solid #1a2a40'>"
        f"<table style='width:100%;border-collapse:collapse;background:#0a0e18'>"
        f"<thead><tr>{header_cells}</tr></thead>"
        f"<tbody>{rows_html}</tbody></table></div>",
        unsafe_allow_html=True,
    )

    # Row selector using st.dataframe (hidden) for click detection
    # Show a tiny 1-col dataframe with just row numbers — fast, no styling
    _sel_df = pd.DataFrame({"#": range(1, len(view_display)+1)})
    st.markdown(
        "<div style='font-family:monospace;font-size:9px;color:#7aaac0;"
        "text-transform:uppercase;letter-spacing:1px;margin:8px 0 3px 0'>"
        "Click row # below to load heatmaps above ↑</div>",
        unsafe_allow_html=True,
    )
    _event = st.dataframe(
        _sel_df,
    ...
    _rows = _event.selection.rows if _event and hasattr(_event, "selection") else []
    _new_idx = _rows[0] if _rows else -1
    if _new_idx != cur_idx:
        st.session_state["_lb_hm_idx"] = None if _new_idx < 0 else _new_idx
        st.rerun()
