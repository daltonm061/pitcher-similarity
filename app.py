...
/* ── HEATMAP EXPANDER ── */
[data-testid="stExpander"] summary span.heatmap-label {
...
def render_zone_heatmap(pitcher_zone_df, stat_col, stat_type, title, fmt=".1%"):
    ...
    svg = (
        ...
        f"border-radius:8px;border:1px solid #162236'>"
        f"<rect width='{TOTAL_W}' height='{TOTAL_H}' fill='#0a0e18' rx='6'/>"
        f"<text x='{TOTAL_W//2}' y='11' text-anchor='middle' "
        f"font-family='JetBrains Mono,monospace' font-size='7' fill='#3a6a8a' "
        f"letter-spacing='1' text-transform='uppercase'>{title.upper()}</text>"
    )
...
def comp_aggregate_stats(results, pitch_group=None):
    ...
    for r in results:
        ...
        # Negate VAA for display: stored positive in CSV, convention is negative = downward approach
        vaa_display = (-float(vaa) if vaa is not None and not (isinstance(vaa, float) and vaa != vaa) else None)
        for k, v in [("velo",velo),("ivb",ivb),("hb",hb_display),("vaa",vaa_display),("haa",haa),("stuff_plus",sp)]:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with lc:
        st.markdown(
            "<div style='text-align:center;padding:32px;background:linear-gradient(165deg,#0e1828,#0a1218);"
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with main_col:
        ...
        st.markdown(
            ...
            "color:#d4a848;letter-spacing:2px;text-transform:uppercase;"
            "margin:0 0 12px 0;padding-bottom:8px;border-bottom:1px solid #1a2a40'>"
            "● TrackMan / Rapsodo Auto-Fill <span style='color:#3a5a78;font-size:9px;font-weight:400;letter-spacing:1px'>(optional)</span></div>",
            unsafe_allow_html=True,
        )
        ...
        if mode == "arsenal":
            ...
            st.markdown(
                ...
                "color:#d4a848;letter-spacing:2px;text-transform:uppercase;"
                "margin:0 0 12px 0;padding-bottom:8px;border-bottom:1px solid #1a2a40'>"
                "● Pitch Arsenal <span style='color:#3a5a78;font-size:9px;font-weight:400;letter-spacing:1px'>fill only the pitches you throw</span></div>",
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                ...
                "color:#d4a848;letter-spacing:2px;text-transform:uppercase;"
                "margin:0 0 12px 0;padding-bottom:8px;border-bottom:1px solid #1a2a40'>"
                "● Single Pitch <span style='color:#3a5a78;font-size:9px;font-weight:400;letter-spacing:1px'>enter your pitch metrics</span></div>",
                unsafe_allow_html=True,
            )
            st.markdown(hint_html, unsafe_allow_html=True)
        ...
        for r in _pdicts_lb:
            ...
            for grp in PITCH_GROUPS:
                ...
                rows.append({
                    ...
                    "VAA":         round(-float(vaa), 1) if is_real(vaa)  else None,  # negate: stored positive, display negative
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def stat_pill(label, val, color, sub=None):
        """Stat pill with dark bg tinted by color — matches leaderboard palette."""
        # Tint background with color at low opacity, value in color
        sub_html = (
            f"<div style='font-family:JetBrains Mono,monospace;font-size:8px;"
    ...
    def render_comp_section(title_label, title_color, groups_to_show):
        st.markdown(
            ...
            f"margin:0 0 12px 0;padding-bottom:8px;"
            f"border-bottom:1px solid #1a2a40'>"
            f"● {title_label}</div>",
            unsafe_allow_html=True,
        )
        ...
        for grp in groups_to_show:
            ...
            _xw_sd   = 0.06
            csw_c    = stat_gradient_color(csw_val,   _csw_mu, _csw_sd)          if csw_val   else "#2a4a5a"
            xw_c     = stat_gradient_color(xw_val,    _xw_mu,  _xw_sd,  invert=True) if xw_val else "#2a4a5a"
            whiff_c  = stat_gradient_color(whiff_val, 0.22,    0.04)              if whiff_val else "#2a4a5a"
            ...
            # Note: vaa_val from agg is already negated for display
            # Use raw stored positive value for color comparison against vaa_mu (also positive)
            _vaa_raw_for_color = -vaa_val if vaa_val is not None else None
            vaa_c = stat_gradient_color(_vaa_raw_for_color, _vl["vaa_mu"], _vl["vaa_sd"], invert=True) if (_vl and _vaa_raw_for_color is not None) else "#2a4a5a"
            ...
            st.markdown(
                ...
                f"color:#4a6880'>comp average across {len(results)} matches</div>"
                f"</div>",
                unsafe_allow_html=True,
            )
            ...
            pills_html = (
                ...
                + stat_pill("STUFF+",
                            ...
                            (f"{user_dmsp[grp]:.0f} yours" if grp in user_dmsp
            ...
            with st.expander(f"📊  {grp} comp zone heatmaps", expanded=False):
                ...
                with hm_col1:
                    st.markdown(render_zone_heatmap(zdf, "csw_pct",   "csw",   "CSW% (All)",   fmt=".1%"),  unsafe_allow_html=True)
                with hm_col2:
                    st.markdown(render_zone_heatmap(zdf, "whiff_pct", "whiff", "Whiff% (All)", fmt=".1%"),  unsafe_allow_html=True)
                with hm_col3:
                    st.markdown(render_zone_heatmap(zdf, "xwoba_mean","xwoba", "xwOBA (All)",  fmt=".3f"),  unsafe_allow_html=True)
                ...
                if has_stand_col:
                    ...
                    if not zdf_same.empty:
                        st.markdown("<div style='font-family:monospace;font-size:9px;color:#7aaac0;"
    ...
    # ── Full-width band separating comp section from pitcher list ───────────
    st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
    st.markdown(
    ...
    for idx, r in enumerate(results):
        ...
        # ── Expander header — clean hierarchy ─────────────────────────────
        score_badge = f"SIM {sc:.1f}"
        pitch_badge = f"· {matched_pitch}" if result_mode == "single" and matched_pitch else ""
        header = (
            f"#{rank}  {full_name}  {r['Year']}  ({hand}HP)  {pitch_badge}  {score_badge}"
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            st.markdown(
                ...
                # Rank badge
                f"<div style='font-family:Inter,sans-serif;font-size:28px;"
                f"font-weight:900;color:{rank_c};opacity:0.18;position:absolute;"
                f"right:16px;top:50%;transform:translateY(-50%);line-height:1'>#{rank}</div>"
                # Left: name + meta
                f"<div style='flex:1;min-width:0'>"
                f"<div style='font-family:Inter,sans-serif;font-size:16px;"
                f"font-weight:800;color:#e8dcc8;letter-spacing:0.5px;"
                f"margin-bottom:4px'>{full_name}</div>"
                f"<div style='font-family:JetBrains Mono,monospace;font-size:10px;"
                ...
                + (f"<span>{int(row.get('total_pitches',0)):,} pitches</span>"
                ...
                f"<div style='font-family:JetBrains Mono,monospace;font-size:8px;"
                f"color:#4a6880;text-transform:uppercase;letter-spacing:1px;"
                f"margin-bottom:4px'>Similarity</div>"
                ...
                f"<div style='font-family:JetBrains Mono,monospace;font-size:8px;"
                f"color:#4a6880;text-transform:uppercase;letter-spacing:1px;"
                f"margin-bottom:4px'>DM Stuff+</div>"
                f"<div style='font-family:Inter,sans-serif;font-size:24px;"
            ...
            if not active:
                ...
                for group in active:
                    ...
                    vaa_s  = f"{-vaa_v:.1f}°"  if is_real(vaa_v)   else "—"  # negate for display
                    ...
                    def _pc(lbl, val, c, sub=None):
                        """Pitch card pill — dark bg tinted by color, value in color."""
                        sub_h = (
                            f"<div style='font-family:JetBrains Mono,monospace;font-size:8px;"
                            f"color:#6a90a8;margin-top:3px;letter-spacing:0.3px'>{sub}</div>"
                        ) if sub else ""
                    ...
                    card_html = (
                        ...
                        + _pc("STUFF+",
                               ...
                               (f"{user_dmsp[group]:.0f} yours" if group in user_dmsp else (sp_lbl if sp_has else "—")))
                        ...
                        with st.expander(f"📊  {group} zone heatmaps", expanded=False):
                            ...
                            if has_stand_col:
                                ...
                                if not pz_same.empty:
                                    st.markdown("<div style='font-family:JetBrains Mono,monospace;font-size:9px;color:#7aaac0;"
                                                "text-transform:uppercase;letter-spacing:1px;margin:10px 0 4px 0'>"
                                                "vs Same Hand</div>", unsafe_allow_html=True)
