...
#
# Tread/TrackMan PDF report column order (the format seen in the wild):
#   PITCHER | PITCH | # | AVG_VELO | MAX_VELO | VERT | HORZ | AVG_SPIN | HEIGHT | SIDE | EXTENSION
#
# After stripping pitch count (small int) and spin rate (>500) the positional
# order of the remaining numbers is always:
#   [0] avg_velo  [1] max_velo  [2] vert(ivb)  [3] horz(hb)
#   [4] height    [5] side      [6] extension
#
# We detect this layout from the header row and fall back to the old heuristic
# parser for any format that doesn't match.

_TREAD_COLS = ["avg velo","max velo","vert","horz","avg spin","height","side","extension"]

def _detect_tread_layout(text_lines):
    """Return True if the OCR text looks like a Tread/TrackMan pitch metrics table."""
    import re
    header_hits = 0
    for line in text_lines:
        ll = line.lower()
        ...
        if hits >= 3:
            return True
        # Also accept "pitch metrics" title as confirmation
        if "pitch metrics" in ll:
            header_hits += 1
    return header_hits >= 1


def _parse_tread_layout(text_lines):
    """
    Column-order parser for Tread/TrackMan pitch metrics reports.
    Expects per-row: PITCH_NAME  #  AVG_VELO  MAX_VELO  VERT  HORZ  AVG_SPIN  HEIGHT  SIDE  EXT
    Returns {group: {velo, ivb, hb, rel_height, rel_side, extension, spin_rate}}
    """
    import re
    # Match numbers but NOT digits embedded in words like "4-Seam" or "2-seam"
    # Use word-boundary aware pattern: number must be preceded by space/start or sign
    ...
    for line in text_lines:
        ...
        # Identify pitch type anywhere in the line
        group = None
        matched_key = ""
        for key, grp in TM_PITCH_MAP.items():
            if key in ll:
        ...
        # Remove the matched pitch name text from the line before number extraction
        # to avoid digits in pitch names (e.g. "4" in "4-Seam") being treated as data
        line_stripped = re.sub(re.escape(matched_key), " ", ll, flags=re.IGNORECASE)
        # Also strip any leading pitcher name text (non-numeric prefix before first digit)
        ...
        # Strip pitch count: first small positive integer with no decimal point in original
        filtered = []
        dropped_count = False
        for n, f in zip(raw_nums, floats):
            if not dropped_count and f == int(f) and 1 <= f <= 999 and "." not in n:
                dropped_count = True   # drop only the first such integer (pitch count)
                continue
            filtered.append(f)

        # Strip spin rate: integer > 500
        no_spin = [f for f in filtered if not (f > 500 and f == int(f))]
        spin    = next((f for f in filtered if f > 500 and f == int(f)), None)

        # After stripping, positional layout is:
        #  [0]=avg_velo  [1]=max_velo  [2]=vert(ivb)  [3]=horz(hb)
        #  [4]=height    [5]=side      [6]=extension
        if len(no_spin) < 2:
            continue
        ...
        # no_spin[1] = max_velo — skip
        vert      = no_spin[2] if len(no_spin) > 2 else None   # iVB
        horz      = no_spin[3] if len(no_spin) > 3 else None   # HB
...
def parse_trackman_image(file_bytes: bytes, filename: str) -> dict:
    """
    Parse a TrackMan/Rapsodo screenshot (JPG/PNG) using Tesseract OCR.
    Detects Tread/TrackMan column-order layout and parses positionally;
    falls back to heuristic parser for other formats.
    Returns same dict format as parse_trackman: {group: {velo, ivb, hb, ...}}

    Requires: pytesseract + Pillow in requirements.txt
              tesseract-ocr in packages.txt (Streamlit Cloud system package)
    """
    try:
        import pytesseract
    ...
    try:
        ...
        # Grayscale → sharpen → contrast boost for clean OCR on report tables
        img = img.convert("L")
        img = ImageEnhance.Contrast(img).enhance(2.5)
        img = img.filter(ImageFilter.SHARPEN)
        img = img.filter(ImageFilter.SHARPEN)   # double-sharpen helps thin fonts
    ...
    # Normalize Unicode minus signs (Tread uses − U+2212)
    text_lines = [l.replace("\u2212", "-").replace("\u2013", "-") for l in text_lines]
    # Remove thousand-separator commas (e.g. 2,303 → 2303)
    text_lines = [re.sub(r"(\d),(\d{3})\b", r"\1\2", l) for l in text_lines]

    # ── Try Tread/TrackMan column-order parser first ──────────────────────────
    if _detect_tread_layout(text_lines):
        results = _parse_tread_layout(text_lines)
        if results:
            return results
        # If column parser found nothing, fall through to heuristic

    # ── Heuristic fallback (generic TrackMan CSV / other report formats) ──────
    num_re = re.compile(r"-?\d+\.\d+|-?\d{2,}")
    results = {}
    parsed_any = False
    ...
    for line in text_lines:
        ...
        # Drop pitch count (small integers ≤ 50, no decimal) and spin (> 500)
        metric_nums = [
            f for f in floats
        ...
        remaining = metric_nums[vi + 1:]
        if remaining and 60 <= remaining[0] <= 105:   # skip max velo
            remaining = remaining[1:]
        ...
        for grp in groups_to_show:
            ...
            pills_html = (
                ...
                + stat_pill("DM S+",
                            ...
                            sp_c if sp_val else (stuff_color(user_dmsp[grp]) if grp in user_dmsp else "#3a4a5a"),
                            (f"{user_dmsp[grp]:.0f} AVG" if grp in user_dmsp
    ...
    for idx, r in enumerate(results):
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            strip = (
                ...
                "letter-spacing:1.2px;font-weight:500'>DM Stuff+</span>"
                ...
                for group in active:
                    ...
                    card_html = (
                        ...
                        + _pc("DM S+",
                               ...
                               sp_color if sp_has else (stuff_color(user_dmsp[group]) if group in user_dmsp else "#3a6a8a"),
                               (f"{user_dmsp[group]:.0f} AVG" if group in user_dmsp
