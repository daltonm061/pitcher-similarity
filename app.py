...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Build button HTML — onclick navigates parent frame to ?lbr=N
    # window.parent.location works in Streamlit Cloud iframes
    btn_rows = ""
    for i_b in range(len(row_labels)):
        cls    = "active" if i_b == cur_idx else ""
        icon   = "&#9654;" if i_b == cur_idx else "&#9711;"
        nav    = "window.parent.location.href=window.parent.location.pathname+'?lbr=" + str(i_b) + "'"
        btn_rows += f'<button class="{cls}" onclick="{nav}">{icon}</button>\n'
