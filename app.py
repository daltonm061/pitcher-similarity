...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Metric range filters — 2 per row
    st.markdown(
        ...
        "Filter by range (leave blank for no limit)</div>",
        unsafe_allow_html=True,
    )
    range_filters = {}
    filter_cols = [c for c in METRIC_COLS if c in lb_df.columns]
    for i in range(0, len(filter_cols), 4):
        row_cols = st.columns(min(4, len(filter_cols) - i) * 2)
        for j, metric in enumerate(filter_cols[i:i+4]):
            col_min = row_cols[j*2]
            col_max = row_cols[j*2+1]
            with col_min:
                mn = st.number_input(f"{metric} min", value=None, key=f"lb_min_{metric}",
                                     label_visibility="visible", format="%.1f")
            with col_max:
                mx = st.number_input(f"{metric} max", value=None, key=f"lb_max_{metric}",
                                     label_visibility="visible", format="%.1f")
    ...
    with sort_col1:
        sort_by = st.selectbox("Sort by", ["Pitcher", "Year", "Pitch"] + filter_cols,
                               key="lb_sort_col", label_visibility="visible")
    with sort_col2:
        sort_dir = st.selectbox("Order", ["↓ Desc", "↑ Asc"],
                                key="lb_sort_dir", label_visibility="visible")
    ...
    st.markdown(
        ...
        f"margin:8px 0 4px 0'>{len(view):,} pitches</div>",
        unsafe_allow_html=True,
    )

    # ── Render leaderboard ────────────────────────────────────────────────────
    # Pre-compute league baselines for gradient coloring
    lb_baselines = {}
    ...
    def cell_color(col, val):
        ...
        # Black text on bright colors, white on dark
        if c.startswith("rgb"):
            try:
                parts = c[4:-1].split(",")
    ...
    # Render as HTML table — fast, gradient colored
    DISPLAY_COLS = ["Pitcher", "Year", "Hand", "Pitch"] + [c for c in filter_cols if c in view.columns]

    # Build table HTML
    header_cells = ""
    for col in DISPLAY_COLS:
        header_cells += (
            f"<th style='padding:6px 8px;text-align:{'left' if col in ('Pitcher','Pitch') else 'center'};"
            f"font-family:JetBrains Mono,monospace;font-size:9px;color:#7aaac0;"
            f"text-transform:uppercase;letter-spacing:1px;border-bottom:1px solid #1a2a40;"
            f"white-space:nowrap;cursor:pointer'>{col}</th>"
        )

    pitch_colors_js = {g: PITCH_COLORS.get(g, "#8aadcc") for g in PITCH_GROUPS}

    rows_html = ""
    display_limit = min(500, len(view))
    for _, row in view.head(display_limit).iterrows():
        row_cells = ""
        for col in DISPLAY_COLS:
            val = row.get(col)
            if col == "Pitcher":
    ...
    if len(view) > display_limit:
        rows_html += f"<tr><td colspan='{len(DISPLAY_COLS)}' style='padding:8px;text-align:center;font-family:JetBrains Mono,monospace;font-size:10px;color:#7aaac0'>Showing top {display_limit} of {len(view):,} — use filters to narrow results</td></tr>"
