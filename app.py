...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Row selector — above heatmaps so click → heatmap is immediately visible
    _sel_df = pd.DataFrame({
        "#":       range(1, len(view_display)+1),
        "Pitcher": view_display["Pitcher"].values,
        "Year":    view_display["Year"].values,
        "Pitch":   view_display["Pitch"].values,
    })
    st.markdown(
        "<div style='font-family:monospace;font-size:9px;color:#7aaac0;"
        "text-transform:uppercase;letter-spacing:1px;margin:4px 0 3px 0'>"
        "Click a row to view heatmaps:</div>",
        unsafe_allow_html=True,
    )
    ...
    _sel_rows = _sel_event.selection.rows if _sel_event and hasattr(_sel_event, "selection") else []
    _sel_new  = _sel_rows[0] if _sel_rows else -1
    _cur_hm   = st.session_state.get("_lb_hm_idx", -1)
    if _sel_new >= 0 and _sel_new != _cur_hm:
        st.session_state["_lb_hm_idx"] = _sel_new
        st.rerun()
    elif _sel_new < 0 and _cur_hm >= 0:
        pass  # keep current selection when no row clicked
    ...
    # ── Sort controls (sort already applied above, just UI) ─────────────────
