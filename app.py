                ...
                if pitches_dict:
                    ...
                    if scores:
                        ...
                        try:
                            ...
                            # Find closest A+ arsenal from pitcher_profiles
                            _pp_mp = st.session_state.get("_profiles_df_cache")
                            if _pp_mp is None:
                                _pp_mp = pd.read_csv("pitcher_profiles.csv")
                                st.session_state["_profiles_df_cache"] = _pp_mp

                            # Compute per-row arsenal avg Stuff+
                            _sp_cols = [f"sp_{g}" for g in _GROUPS_ALL if f"sp_{g}" in _pp_mp.columns]
                            _pp_mp = _pp_mp.copy()
                            _pp_mp["_avg_sp"] = _pp_mp[_sp_cols].mean(axis=1, skipna=True)
                            _aplus_pp = _pp_mp[_pp_mp["_avg_sp"] >= 112].copy()

                            # Filter to same throwing hand
                            _hand_char = "L" if dm_hand == "LHP" else "R"
                            _aplus_pp = _aplus_pp[_aplus_pp["hand"] == _hand_char]

                            _user_grps = list(_user_pts.keys())
                            _best_row  = None
                            _best_dist = float("inf")

                            # User release profile for matching
                            _u_rh  = dm_rh  if dm_rh  is not None else _V5_MEDIANS["rel_height"]
                            _u_rs  = dm_rs  if dm_rs  is not None else abs(_V5_MEDIANS["rel_side_arm"])

                            for _, _row in _aplus_pp.iterrows():
                                # Check how many user pitch types this pitcher has
                                _overlap = [g for g in _user_grps
                                            if f"ivb_{g}" in _row.index
                                            and pd.notna(_row.get(f"ivb_{g}"))
                                            and pd.notna(_row.get(f"hb_{g}"))]
                                if not _overlap:
                                    continue
                                # Movement distance across overlapping pitches.
                                # Profile HB is glove-side positive; user entries are arm-side
                                # positive — negate profile HB to compare in the same frame.
                                _dist = 0.0
                                for _g in _overlap:
                                    _dh = (-float(_row[f"hb_{_g}"])) - _user_pts[_g][0]
                                    _di = float(_row[f"ivb_{_g}"]) - _user_pts[_g][1]
                                    _dist += (_dh**2 + _di**2) ** 0.5
                                _dist /= len(_overlap)
                                # Release profile penalty: scale ft → inches equivalent
                                _p_rh = _row.get("rel_height", _u_rh)
                                _p_rs = _row.get("rel_side",   _u_rs)
                                if pd.notna(_p_rh): _dist += abs(float(_p_rh) - _u_rh) * 8.0
                                if pd.notna(_p_rs): _dist += abs(float(_p_rs) - _u_rs) * 8.0
                                # Penalize for missing pitch types
                                _dist += (len(_user_grps) - len(_overlap)) * 15.0
                                if _dist < _best_dist:
                                    _best_dist = _dist
                                    _best_row  = _row

                            # Build matched pitcher points — negate HB so it's arm-side positive
                            _match_pts = {}
                            _match_name = ""
                            if _best_row is not None:
                                _match_name = f"{_best_row.get('player_name','?')} ({int(_best_row.get('year', 0))})"
                                for _g in _GROUPS_ALL:
                                    _hc = f"hb_{_g}"; _ic = f"ivb_{_g}"
                                    if _hc in _best_row.index and _ic in _best_row.index:
                                        _hv = _best_row[_hc]; _iv = _best_row[_ic]
                                        if pd.notna(_hv) and pd.notna(_iv):
                                            _match_pts[_g] = (-float(_hv), float(_iv))
                            ...
                            _legend_elems = [
                                _L2D([0],[0], marker="o", color="w", markerfacecolor="#aaaaaa",
                                     markersize=9, label="Your arsenal", linestyle="None"),
                                _L2D([0],[0], marker="D", color="w", markerfacecolor="none",
                                     markeredgecolor="#aaaaaa", markersize=9,
                                     label=f"A+ match: {_match_name}" if _match_name else "A+ match",
                                     linestyle="None"),
                            ]
                            ...
                            if _match_name:
                                st.markdown(
                                    f"<div style='font-family:JetBrains Mono,monospace;font-size:10px;"
                                    f"color:#3a5a78;margin-top:6px;padding:0 4px'>"
                                    f"Closest A+ match (same hand, min movement distance): "
                                    f"<b style='color:#a0c0d4'>{_match_name}</b> "
                                    f"· avg arsenal Stuff+ {_best_row['_avg_sp']:.1f}"
                                    f"</div>",
                                    unsafe_allow_html=True,
                                )
