...
# ── Zone-Stuff+ v1 model loader (optional second model) ────────────────────
_ZONE_BUNDLE_PATH = "models/zone_stuff_v1.joblib"

def load_zone_stuff_v1():
    """Load zone-conditional Stuff+ bundle, or return None if missing/invalid."""
    if not _os.path.exists(_ZONE_BUNDLE_PATH):
        return None
    try:
        b = _joblib.load(_ZONE_BUNDLE_PATH)
    except Exception as _e:
        print(f"  ! failed to load zone-Stuff+ bundle: {_e}")
        return None
    # Validate bundle has what we need
...
_zone_bundle = load_zone_stuff_v1()
...
def _score_v5_arsenal(pitches: dict, rel_height: float = None,
    ...
    if primary_velo is None:
        # Fall back to highest-velo pitch in the arsenal
        try:
            best = max(((g, m) for g, m in pitches.items() if m.get("velo") is not None),
                        key=lambda kv: float(kv[1]["velo"]),
                        default=(None, None))
            if best[1] is not None:
                primary_velo = float(best[1]["velo"])
                primary_ivb  = (float(best[1]["ivb"]) if best[1].get("ivb") is not None
                                 else _V5_MEDIANS["ivb_in"])
                primary_hb   = ((-float(best[1]["hb"])) if best[1].get("hb") is not None
                                 else _V5_MEDIANS["hb_arm_in"])
        except (TypeError, ValueError):
            primary_velo = _V5_MEDIANS["start_speed"]
            primary_ivb  = _V5_MEDIANS["ivb_in"]
            primary_hb   = _V5_MEDIANS["hb_arm_in"]
    ...
    for grp, m in pitches.items():
        ...
        # VAA/HAA: in this form we don't have trajectory, so use medians (imputed)
        vaa = _V5_MEDIANS["vaa"];  imputed.append("vaa")
