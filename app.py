...
def comp_aggregate_stats(results, pitch_group=None):
    ...
    for r in results:
        ...
        # Convert HB from pfx_x to TrackMan display convention before averaging
        # RHP: display = -pfx_x,  LHP: display = +pfx_x
        hb_tm = (-float(hb) if hand_r == "R" else float(hb)) if _safe(hb) else None
        for k, v in [("velo",velo),("ivb",ivb),("hb",hb_tm),("vaa",vaa_display),("haa",float(haa) if _safe(haa) else None),("stuff_plus",sp)]:
