...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Calculate header height to match
    HDR_H = 33  # px — matches thead row
    ROW_H = 33  # px — matches tbody row

    btns_html = f"""
    <style>
      body {{ margin:0; padding:0; background:#0a0e18; overflow:hidden; }}
      .hdr {{ height:{HDR_H}px; background:#0c1420; border-bottom:1px solid #1a2a40;
              display:flex; align-items:center; justify-content:center;
              font-family:monospace; font-size:8px; color:#7aaac0;
              text-transform:uppercase; letter-spacing:1px; }}
      button {{ display:block; width:100%; height:{ROW_H}px;
      ...
      button.active {{ background:#162236; color:#d4a848;
    ...
    for i_b in range(len(row_labels)):
        ...
        btns_html += '<button class="' + cls + '" onclick="handleClick(' + str(i_b) + ')">' + icon + '</button>\n'
    ...
    total_h = HDR_H + ROW_H * len(row_labels) + 10
    btn_col, tbl_col = st.columns([0.35, 11])

    with btn_col:
        clicked = st.components.v1.html(btns_html, height=total_h, scrolling=False)
        if clicked is not None and isinstance(clicked, (int, float)):
            idx_c = int(clicked)
            if idx_c == st.session_state.get("_lb_hm_idx"):
    ...
    with tbl_col:
        table_html = (
            ...
            "border:1px solid #1a2a40;border-left:none;margin-top:0'>"
