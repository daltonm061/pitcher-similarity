...
# ── Session state ─────────────────────────────────────────────────────────────
for k, v in [("screen","input"), ("results",None), ("user_snapshot",{}), ("mode","arsenal")]:
...
def render_zone_heatmap(pitcher_zone_df, stat_col, stat_type, title, fmt=".1%"):
    ...
    for (zone_id, gr, gc) in ALL_ZONES:
        ...
        txt_fill = "#e8dcc8" if val is not None else ("#1a3a5a" if is_inside else "#0f2535")
...
def run_search_single_pitch(user, velo, ivb, hb_csv, top_n):
    """
...
if st.session_state.screen == "input":
    ...
    with mode_col:
        st.markdown(
            ...
            "text-align:center;margin-bottom:10px'>Search Mode</div>",
            unsafe_allow_html=True,
        )
        mode_choice = st.radio(
            "_mode",
            ["⚾  Full Arsenal  —  match an entire pitch mix",
             "🎯  Single Pitch  —  find who throws this pitch"],
            index=0 if st.session_state.mode == "arsenal" else 1,
            key="mode_radio",
            label_visibility="collapsed",
            horizontal=True,
        )
        mode = "arsenal" if "Arsenal" in mode_choice else "single"
    ...
    subtitle = (
        "Leave any field blank = open filter &nbsp;·&nbsp; Fill only the pitches you throw"
        if mode == "arsenal" else
        "Enter one pitch's metrics — the app searches every pitch type across all pitchers"
    )
    st.markdown(
    ...
    with main_col:
        ...
        def pitch_inputs_widget(group, key_prefix, tm_data, show_placeholder=False):
            ...
            velo = st.number_input(f"_{key_prefix}_velo", min_value=60.0, max_value=105.0,
            ...
            ivb = st.number_input(f"_{key_prefix}_ivb", min_value=-30.0, max_value=30.0,
            ...
            hb = st.number_input(f"_{key_prefix}_hb", min_value=-30.0, max_value=30.0,
        ...
        pitch_inputs_raw = {}
        sp_velo = sp_ivb = sp_hb_csv = None   # single-pitch mode values
        ...
        if mode == "arsenal":
            ...
            st.markdown(hint_html, unsafe_allow_html=True)
            # First pick from TrackMan if available (use first parsed pitch)
            first_tm = next(iter(tm_data.values()), {}) if tm_data else {}
            sp_col, _ = st.columns([3, 6])
            with sp_col:
                sp_velo, sp_ivb, sp_hb_csv = pitch_inputs_widget(
                    "4-Seam", "sp", {"4-Seam": first_tm}, show_placeholder=True
                )
        ...
        with btn_col:
            ...
            btn_label = "⚾  Find My MLB Comps" if mode == "arsenal" else "🎯  Find This Pitch"
        ...
        if run:
            ...
            if mode == "arsenal":
                ...
                if sp_velo is None and sp_ivb is None and sp_hb_csv is None and not any(v is not None for v in user.values()):
                    ...
                    with st.spinner("Searching all pitch types…"):
                        results = run_search_single_pitch(user, sp_velo, sp_ivb, sp_hb_csv, top_n)
                    st.session_state.results = results
                    st.session_state.screen  = "results"
                    st.rerun()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def stat_pill(label, val, color, sub=None):
        sub_html = f"<div style='font-family:monospace;font-size:8px;color:{color}88;margin-top:1px'>{sub}</div>" if sub else ""
        return (
            ...
            f"<div style='font-family:monospace;font-size:8px;color:#3a6a8a;"
