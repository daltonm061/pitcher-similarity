...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    total_rows   = len(view)
    view_display = view  # sort applied later after sort buttons render
    ...
    sort_btn_cols = st.columns([0.5] + [1.0] * len(sort_cols_list))
    with sort_btn_cols[0]:
        st.markdown(
            "<div style='font-family:monospace;font-size:9px;color:#7aaac0;"
            "text-transform:uppercase;letter-spacing:1px;padding:8px 4px 0 0;"
            "text-align:right'>Sort</div>",
            unsafe_allow_html=True,
        )
    for j, col in enumerate(sort_cols_list):
        with sort_btn_cols[j+1]:
    ...
    _hm_idx = cur_idx if 0 <= cur_idx < len(view_display) else None
    ...
    _pa = df_show["Pitch"].values if "Pitch" in df_show.columns else None
    _ha = df_show["Hand"].values  if "Hand"  in df_show.columns else None

    # Build style df — one CSS string per cell, only gradient cols filled
    _style = pd.DataFrame("", index=df_show.index, columns=df_show.columns)
    for _col in gradient_cols:
        _vals = df_show[_col].values.astype(float)
        ...
        if _col in ("VAA","HAA"):
            ...
            _lb = st.session_state.get("_lb_rel_base") or _hand_release_baselines(len(profiles))
            st.session_state["_lb_rel_base"] = _lb
    ...
    styled = df_show.style.apply(lambda _: _style, axis=None).format(fmt, na_rep="—")
