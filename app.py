...
def comp_aggregate_stats(results, pitch_group=None):
    ...
    for r in results:
        ...
        # Convert HB to arm-side positive before averaging (consistent display convention)
        # RHP: arm-side = -pfx_x,  LHP: arm-side = +pfx_x
        hb_arm = (-float(hb) if hand_r == "R" else float(hb)) if _safe(hb) else None
...
def hb_to_csv(user_hb, pitcher_hand="R"):
    """Convert user HB (arm-side positive, TrackMan convention) to pfx_x for CSV comparison.
    RHP: arm-side = toward 3B = negative pfx_x  → csv = -user_hb
    LHP: arm-side = toward 1B = positive pfx_x  → but pfx_x LHP arm-side is ALSO negative
         because LHP fastball pfx_x = -7.7 (toward 3B), same as RHP.
         TrackMan LHP arm-side = negative → csv = pfx_x = user_hb (no flip for LHP).
    Note: since both hands store same pfx_x for same pitch shape, entering +10 (RHP)
    or -10 (LHP) both produce csv=-10 and match BOTH RHP and LHP fastballs.
    """
    if user_hb is None: return None
    if pitcher_hand == "L":
        return float(user_hb)   # LHP arm-side negative → pfx_x = user_hb
    return -float(user_hb)      # RHP arm-side positive → pfx_x = -user_hb
...
def score_row(user, pitch_inputs, row):
    ...
    for group, metrics in pitch_inputs.items():
        ...
        if has_pitch:
            ...
            user_hb_raw = metrics.get("hb")   # Statcast convention
            user_ivb    = metrics.get("ivb")
            mv_hb       = row.get(f"hb_{col_group}")
            mv_ivb      = row.get(f"ivb_{col_group}")
            # Convert user HB (arm-side positive) to pfx_x convention for this pitcher's hand
            pitcher_hand = row.get("hand", "R")
            user_hb = hb_to_csv(user_hb_raw, pitcher_hand)
        ...
        for group in search_groups:
            ...
            if ivb is not None and is_real(mv_ivb):
                if abs(float(mv_ivb) - float(ivb)) > _ivb_thresh:
                    continue
            # Convert user HB (arm-side positive) to pfx_x for this pitcher's hand
            _hb_use = hb_to_csv(hb_csv, r.get("hand", "R"))
            if hb_csv is not None and is_real(mv_hb):
                if abs(float(mv_hb) - (_hb_use or 0)) > _hb_thresh:
                    continue
            # Score this pitch
            p_log = log_sum
            p_w   = total_w
            for metric, mv, sigma in [
                ...
                if metric == "hb"   and hb_csv is None:
                    continue
                user_val = {"velo": velo, "ivb": ivb, "hb": _hb_use if _hb_use is not None else hb_csv}[metric]
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def _make_clr_max(c):
        ...
        return _fn
    def _cb_max_num(c):
        v = st.session_state.get(f"_w_max_{c}")
        if v is None:
            st.session_state["_lbf"].pop(f"max_{c}", None)
        else:
    ...
    for j, col in enumerate(CTRL_COLS):
        with row_max[j+1]:
            ...
            st.number_input(
                ...
                key=f"_w_max_{col}", on_change=_cb_max_num, args=(col,),
                label_visibility="collapsed",
            )
    ...
    def _make_clr_min(c):
        ...
        return _fn
    def _cb_min_num(c):
        v = st.session_state.get(f"_w_min_{c}")
        if v is None:
            st.session_state["_lbf"].pop(f"min_{c}", None)
        else:
    ...
    for j, col in enumerate(CTRL_COLS):
        with row_min[j+1]:
            ...
            st.number_input(
                ...
                key=f"_w_min_{col}", on_change=_cb_min_num, args=(col,),
                label_visibility="collapsed",
            )
                    ...
                    if _col == "VAA":
                        ...
                        if _pb and "hb_mu" in _pb:
                            ...
                            if _pt == "2-Seam/Sinker":
                                # RHP: more positive HB = more run = red
                                # LHP: more negative HB = more run = higher_is_better=False
                                _bg, _tx = _directional_color(_vals[_i], _pb["hb_mu"], max(_pb["hb_sd"],0.001), higher_is_better=(_hd != "L"))
                                _css[_i] = f"background-color:{_bg};color:{_tx}"
                            else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for idx, r in enumerate(results):
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            def display_hb(raw_hb, pitcher_hand="R"):
                """Display HB as arm-side positive (TrackMan convention).
                RHP: arm-side = toward 3B = negative pfx_x → display = -pfx_x (positive)
                LHP: arm-side = toward 1B = but pfx_x LHP arm-side is -7.7 (toward 3B)
                     TrackMan LHP arm-side = negative → display = pfx_x (negative)
                So: RHP display = -pfx_x, LHP display = +pfx_x
                """
