...
def score_row(user, pitch_inputs, row):
    ...
    for group, metrics in pitch_inputs.items():
        ...
        if has_pitch:
            ...
            # When hand is not constrained, try user HB as-is AND mirrored
            # so e.g. -5 HB matches both RHP cutter (-5 CSV) and LHP cutter (-5 CSV)
            # and also handles case where user enters arm-side positive (+5 = RHP arm-side)
            # by mirroring to -5 which matches both
            if user_hb_raw is not None and is_real(mv_hb) and not user.get("hand"):
                direct   = abs(float(mv_hb) - float(user_hb_raw))
                mirrored = abs(float(mv_hb) - (-float(user_hb_raw)))
                best_hb_diff = min(direct, mirrored)
                user_hb = -float(user_hb_raw) if mirrored < direct else float(user_hb_raw)
            else:
                best_hb_diff = abs(float(mv_hb) - float(user_hb_raw)) if user_hb_raw is not None and is_real(mv_hb) else 0.0
                user_hb = user_hb_raw
            if user_hb_raw is not None and is_real(mv_hb):
                if best_hb_diff > 6.0:
                    has_pitch = False
                    alias_group = None
    ...
    for r in _pdicts:
        ...
        for group in search_groups:
            ...
            if hb_csv is not None and is_real(mv_hb):
                direct_d   = abs(float(mv_hb) - float(hb_csv))
                mirrored_d = abs(float(mv_hb) - (-float(hb_csv)))
                if not user.get("hand"):
                    _hb_use  = -float(hb_csv) if mirrored_d < direct_d else float(hb_csv)
                    _hb_diff = min(direct_d, mirrored_d)
                else:
                    _hb_use  = float(hb_csv)
                    _hb_diff = direct_d
                if _hb_diff > _hb_thresh:
                    continue
            else:
                _hb_use = hb_csv
            # Score this pitch
            ...
            for metric, mv, sigma in [
                ...
                if metric == "hb"   and hb_csv is None:
                    continue
                user_val = _hb_use if metric == "hb" else {"velo": velo, "ivb": ivb}[metric]
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for g, m in pitch_inputs.items():
        ...
        if m.get("hb"):
            _hb_disp = -m['hb'] if snap.get("hand","R") == "R" else m['hb']
            subs.append(f"HB {_hb_disp:+.1f}\"")
        ...
        with st.expander(header, expanded=(idx == 0)):
            ...
            if not active:
                ...
                for group in active:
                    ...
                    _u_hb_disp = display_hb(_u_hb_raw, hand) if _u_hb_raw is not None else None
                    u_hb   = f"{_u_hb_disp:+.1f}\" you" if _u_hb_disp is not None else ""
