...
def _score_arsenal_model(pitches_dict, rel_height=None, rel_side=None, extension=None):
    """Score a full arsenal through the dedicated arsenal-level Stuff+ model.
            ...
            if _do_score and dm_added:
                ...
                if pitches_dict:
                    ...
                    if scores:
                        ...
                        if _scored_vals:
                            # Try arsenal-level model first; fall back to simple avg
                            _asp_model, _grade_model = _score_arsenal_model(
                                pitches_dict,
                                rel_height=dm_rh,
                                rel_side=dm_rs,
                                extension=dm_ext,
                            )
                            if _asp_model is not None:
                                _arsenal_sp = _asp_model
                                _grade = _grade_model
                                _grade_subtitle = "Arsenal-level model · usage-weighted · trained 2017-2025"
                            else:
                                ...
                                _grade_subtitle = "Unweighted avg across entered pitches"

                            if _arsenal_sp >= 107:   _grade_color = "#d4a848"
                        ...
                        # MLB medians used by both movement plot and improvement suggestions
                        # HB values are in arm-side positive convention (same as calculator input).
                        # Profiles store glove-side positive, so these are the negated medians.
                        _MLB_PITCH_MEDIANS = {
