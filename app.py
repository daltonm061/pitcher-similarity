...
def score_tjstuff(pitches: dict, rel_height: float = 6.0,
                  rel_side: float = -1.5, extension: float = 6.2) -> dict:
    """Score entered pitches through TJStuff+ pipeline.
    pitches: {pitch_group: {velo, az, ax, spin_rate}}  (az/ax/spin optional)
    Returns {pitch_group: tjStuff+_score}
    """
    if _tj_model is None:
        return {}
    # Primary pitch = highest velo (fastball) for differential features
    primary   = max(pitches.items(), key=lambda kv: kv[1].get("velo", 0), default=(None, {}))
    p_speed   = primary[1].get("velo",  0)     or 0
    p_az      = primary[1].get("az",  -18.0)   or -18.0
    p_ax      = primary[1].get("ax",    0.0)   or 0.0
    rows, keys = [], []
    for grp, m in pitches.items():
        velo = m.get("velo")
        if velo is None:
            continue
        az = m.get("az", -18.0) or -18.0
        ax = m.get("ax",   0.0) or  0.0
        rows.append({
            "start_speed": velo,
            "spin_rate":   m.get("spin_rate", 2200.0) or 2200.0,
            "extension":   extension,
            "az":          az,
            "ax":          ax,
            "x0":          rel_side,
    ...
    try:
        xrv = _tj_model.predict(X)
    except Exception:
        return {}
    # Standardize: mean=0, SD=0.68 xRV/100 → tjStuff+ mean=100 SD=10
    tjsp = 100.0 - ((xrv - 0.0) / 0.68) * 10.0
    ...
    if _tj_model is not None and data_ok:
        try:
            ...
            def _infer_az_ax(velo, ivb_in, hb_in):
                ...
                t   = 55.0 / (velo * 1.467)
                t2h = t * t / 2.0
                az  = (ivb_in / 12.0) / t2h - 32.174 if ivb_in is not None else -18.0
                ax  = (hb_in  / 12.0) / t2h           if hb_in  is not None else  0.0
            ...
            def _knn_az_ax(grp, velo, ivb_in, hb_in, all_results, n=10):
                """Find the n closest individual pitches of this type from ALL profiles
                (not just the similarity results), sorted by distance in velo/iVB/HB.
                ...
                # Search ALL profiles for this pitch type, not just similarity results
                candidates = []
                for _, prow in profiles.iterrows():
                    ...
                    raz = prow.get(az_col)
                    rax = prow.get(ax_col)
                    if any(v is None or (isinstance(v, float) and _m.isnan(v))
                           for v in [rv, ri, rh, raz, rax]):
                        continue
                    # Distance in velo/iVB/HB space — normalize by typical SD
                    dist = (
                        ((velo   - rv) / 3.0) ** 2 +
                        ((ivb_in - ri) / 3.0) ** 2 +
                        ((hb_in  - rh) / 3.0) ** 2
                    ) ** 0.5
                    candidates.append((dist, float(raz), float(rax)))
                if not candidates:
                    return None, None, None
                candidates.sort(key=lambda x: x[0])
                top = candidates[:n]
                # Inverse-distance weighted average — closer matches count more
                import numpy as _np
                dists  = _np.array([x[0] for x in top])
                az_arr = _np.array([x[1] for x in top])
                ax_arr = _np.array([x[2] for x in top])
                # Avoid division by zero for exact matches
                weights = 1.0 / _np.maximum(dists, 0.01)
                weights /= weights.sum()
                mean_dist = float(dists.mean())
                return float((weights * az_arr).sum()),                        float((weights * ax_arr).sum()),                        mean_dist

            def _best_az_ax(grp, velo, ivb_in, hb_in, all_results):
                """Blend physics (60%) + kNN (40%).
                When kNN matches start to drift (mean_dist increases),
                reduce kNN weight smoothly toward 0% at dist=8 (very poor match).
                Physics gets correspondingly higher weight as matches drift.
                """
            ...
            if mode == "arsenal":
                for _grp, _pm in snap["pitch_inputs"].items():
                    if _pm.get("velo") is None:
                        continue
                    _velo = _pm["velo"]
                    _ivb  = _pm.get("ivb",  0.0) or 0.0
                    _hb   = _pm.get("hb",   0.0) or 0.0
                    _az, _ax = _best_az_ax(_grp, _velo, _ivb, _hb, results)
                    _score_pitches[_grp] = {
                        "velo": _velo, "az": _az, "ax": _ax, "spin_rate": 2200.0,
                    }
            else:
                _sp_type = snap.get("sp_pitch_type") or "4-Seam"
                _velo    = snap.get("sp_velo")
                if _velo is not None:
                    _ivb = snap.get("sp_ivb",   0.0) or 0.0
                    _hb  = snap.get("sp_hb_csv", 0.0) or 0.0
                    _az, _ax = _best_az_ax(_sp_type, _velo, _ivb, _hb, results)
                    _score_pitches[_sp_type] = {
                        "velo": _velo, "az": _az, "ax": _ax, "spin_rate": 2200.0,
                    }

            _rh  = snap["user"].get("rel_height") or 6.0
            _rs  = snap["user"].get("rel_side")   or -1.5
            _ext = snap["user"].get("extension")  or 6.2
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def cell_color(col, val, pitch_type=None, hand=None):
        ...
        if col in ("VAA", "HAA") and pitch_type and pitch_type in lb_vaa_haa:
            league = lb_vaa_haa[pitch_type]
            if col == "VAA":
                return _outlier_color(val, league["vaa_mu"], league["vaa_sd"])
            else:
                return _outlier_color(val, league["haa_mu"], league["haa_sd"])
        elif col in ("VAA", "HAA"):
    ...
    def render_comp_section(title_label, title_color, groups_to_show):
        ...
        for grp in groups_to_show:
            ...
            pills_html = (
                ...
                + stat_pill("TJ S+",  fv("stuff_plus","i"),    sp_c if sp_val else "#3a4a5a",
                            (f"{user_tjsp[grp]:.0f} you" if grp in user_tjsp else None)
                            or (stuff_grade_label(sp_val) if sp_val else "—"))
            ...
            if not active:
                ...
                for group in active:
                    ...
                    card_html = (
                        ...
                        + _pc("TJ S+",  sp_str,   sp_color,
                               (f"{user_tjsp[group]:.0f} you" if group in user_tjsp else None)
                               or (sp_lbl if sp_has else "—"))
