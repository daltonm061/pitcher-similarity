...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for c in df_show.columns:
        if c in ("CSW%","Whiff%"):                               col_cfg[c] = st.column_config.NumberColumn(c, format="%.1f%%")
        elif c == "xwOBA":                                       col_cfg[c] = st.column_config.NumberColumn(c, format="%.3f")
        elif c in ("Stuff+","N"):                                col_cfg[c] = st.column_config.NumberColumn(c, format="%d")
        elif c in ("HB","VAA","HAA"):                            col_cfg[c] = st.column_config.NumberColumn(c, format="%+.1f")
        elif c in ("Velo","iVB","Rel Ht","Rel Side","Extension"):col_cfg[c] = st.column_config.NumberColumn(c, format="%.1f")
