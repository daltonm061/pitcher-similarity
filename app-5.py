...
# ── Constants ─────────────────────────────────────────────────────────────────
STATCAST_YEARS = list(range(2017, 2025))   # 8 seasons
...
PITCH_COLORS = {
    "4-Seam": "#e63946", "2-Seam/Sinker": "#f4a261", "Cutter": "#2a9d8f",
    "Slider": "#457b9d", "Sweeper": "#a855f7", "Splitter": "#e9c46a", "Changeup": "#06d6a0",
}
...
# ── Session state ─────────────────────────────────────────────────────────────
for k, v in [("screen", "loading"), ("results", None), ("user_snapshot", {}), ("profiles", None)]:
...
# ══════════════════════════════════════════════════════════════════════════════
# DATA PIPELINE
# Each year is fetched, immediately aggregated into profiles, then raw data
# is discarded — keeps memory low and avoids Streamlit Cloud OOM crashes.
# @st.cache_data per year so re-runs skip already-loaded seasons.
# ══════════════════════════════════════════════════════════════════════════════

@st.cache_data(show_spinner=False)
def fetch_and_profile_year(year: int) -> pd.DataFrame:
    """
    Fetch one season, build pitcher-season profiles, return only the profile
    DataFrame (not the raw pitch data). This keeps memory usage per year small.
    """
    ...
    # Release profile per pitcher-season
    rel = (
    ...
    # Pitch-type metrics per pitcher-season
    pp = (
...
if st.session_state.screen == "loading":
    ...
    st.markdown("""
    <div style="text-align:center;max-width:560px;margin:0 auto 28px auto">
      <div style="font-family:'Rajdhani',sans-serif;font-size:20px;color:#c9a84c;
                  letter-spacing:2px;text-transform:uppercase;margin-bottom:10px">
        Building Pitcher Database
      </div>
      <div style="font-size:13px;color:#3a6a8a;line-height:1.8">
        Loading 2017–2024 Statcast data from Baseball Savant.<br>
        This happens <strong style="color:#8aadcc">once per server boot</strong> — all searches after are instant.
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for i, year in enumerate(STATCAST_YEARS):
        status.markdown(
            f"<div style='text-align:center;font-family:monospace;font-size:12px;color:#5a8aaa'>"
            f"Fetching {year} season… ({i+1} of {len(STATCAST_YEARS)})</div>",
            unsafe_allow_html=True,
        )

        profile_df = fetch_and_profile_year(year)
        all_profiles.append(profile_df)
        loaded_years.append(year)

        pct = int(((i + 1) / len(STATCAST_YEARS)) * 100)
        prog.progress(pct)

        # Render year chips
        chips_html = "<div style='display:flex;flex-wrap:wrap;gap:8px;justify-content:center;margin-top:12px'>"
        for yr in STATCAST_YEARS:
            done  = yr in loaded_years
            color = "#c9a84c" if done else "#2a4a6a"
            bg    = "#0f2030" if done else "#0b1a2e"
            border= "#c9a84c60" if done else "#1a3550"
            tick  = "✓ " if done else ""
            chips_html += (
                f"<span style='font-family:Rajdhani,sans-serif;font-size:13px;font-weight:600;"
                f"letter-spacing:1px;padding:4px 14px;border-radius:20px;"
                f"border:1px solid {border};color:{color};background:{bg}'>{tick}{yr}</span>"
            )
        chips_html += "</div>"
        year_chips.markdown(chips_html, unsafe_allow_html=True)

    # Combine all years and store
    status.markdown(
        "<div style='text-align:center;font-family:monospace;font-size:12px;color:#5a8aaa'>"
        "Combining profiles…</div>",
        unsafe_allow_html=True,
    )
...
# ── Profiles must be loaded before any other screen ──────────────────────────
profiles: pd.DataFrame = st.session_state.get("profiles")

if profiles is not None:
    n_seasons  = profiles["year"].nunique()
    n_seasons_str = f"{int(profiles['year'].min())}–{int(profiles['year'].max())}"
    st.markdown(
        f"<div style='background:#0a1828;border-bottom:1px solid #1a3550;padding:5px 48px;"
        f"font-family:monospace;font-size:11px;color:#2a5a7a;display:flex;gap:20px'>"
        f"<span>✓ <b style='color:#2a8a5a'>{n_seasons} seasons</b> loaded</span>"
        f"<span>·</span>"
        f"<span><b style='color:#2a8a5a'>{profiles.shape[0]:,}</b> pitcher-seasons indexed</span>"
        f"<span>·</span>"
...
if st.session_state.screen == "input":
    ...
    with main_col:
        ...
        # ── PITCH ARSENAL ─────────────────────────────────────────────────────
        st.markdown(
            '<div class="sec-label">Pitch Arsenal — only fill the pitches you throw</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            "<div class='hb-note'>"
            "<b style='color:#8aadcc'>HB (in):</b> Positive = arm-side run, Negative = glove-side. "
            "Examples: RHP 4-seam tailing <b style='color:#e8dcc8'>+14\"</b>, "
            "RHP slider cutting <b style='color:#e8dcc8'>−4\"</b>, "
            "LHP changeup running <b style='color:#e8dcc8'>+12\"</b>"
            "</div>",
            unsafe_allow_html=True,
        )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    styled = (
        ...
        .set_properties(**{"background-color": "#0b1a2e", "color": "#c8d8e8",
                           "border": "1px solid #1a3550", "font-family": "Georgia, serif",
                           "font-size": "13px"})
