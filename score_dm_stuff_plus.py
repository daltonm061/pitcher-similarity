...
def _load(model_dir="models"):
    ...
    try:
        ...
        _ARSENAL_CELL_STATS = b.get("arsenal_cell_stats")  # v5d
...
def score_dm_stuff_plus_batch(df_pitches, model_dir="models", recalibrate=True):
    ...
    # The batch path receives profile-aggregated data (one row per pitcher-year-pitch_group).
    # If columns are already present (e.g., pre-computed upstream), skip.
    _ARSENAL_FEATS = [
    ...
    needs_arsenal = any(f not in df.columns for f in _ARSENAL_FEATS) and \
                     all(c in df.columns for c in ["pitcher", "year", "start_speed", "ivb_in", "hb_arm_in"])
    if needs_arsenal:
        # Build per-pitcher-year arsenal from the input data itself
        for f in _ARSENAL_FEATS:
            df[f] = np.nan
        # Group by pitcher-year and compute arsenal context per group
        for (pid, yr), grp_df in df.groupby(["pitcher", "year"], sort=False):
            if len(grp_df) < 2:
                continue
            # arsenal: list of (pitch_type_int, velo, ivb, hb) per pitch type in this group
            arsenal = [
            ...
            for idx, row in grp_df.iterrows():
                ...
                df.at[idx, "arsenal_size"] = len(arsenal)
                if not others:
                    continue
    ...
    _arsenal_set = set(_ARSENAL_FEATS)
