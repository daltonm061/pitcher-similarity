...
.loading-title {
    font-family: 'Rajdhani', sans-serif; font-size: 20px; color: #c9a84c;
    letter-spacing: 3px; text-transform: uppercase; margin-bottom: 8px;
}
.loading-sub { font-size: 13px; color: #3a6a8a; margin-bottom: 32px; line-height: 1.7; }
.loading-year-grid {
...
.year-chip {
    ...
    border: 1px solid #1a3550; color: #3a6a8a; background: #0b1a2e;
}
.year-chip.done { border-color: #c9a84c80; color: #c9a84c; background: #0f2030; }
...
# ══════════════════════════════════════════════════════════════════════════════
# DATA LOADING — boot-time, shared across ALL users via cache_resource
# This runs ONCE when the server starts, then every user hits the cached version.
# ══════════════════════════════════════════════════════════════════════════════

def _fetch_one_year(year: int) -> pd.DataFrame:
    """Fetch and clean a single year of Statcast data."""
...
def _build_profiles(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate pitch-level data into per pitcher-season profiles."""
...
def load_all_profiles() -> pd.DataFrame:
    """
    Fetch all years in parallel and build pitcher-season profiles.
    cache_resource = stored once in server memory, shared across all user sessions.
    Re-used on every subsequent visit — no re-downloading.
    """
    year_dfs = {}
    with ThreadPoolExecutor(max_workers=len(STATCAST_YEARS)) as executor:
        futures = {executor.submit(_fetch_one_year, yr): yr for yr in STATCAST_YEARS}
        for future in as_completed(futures):
            yr = futures[future]
            year_dfs[yr] = future.result()

    combined = pd.concat([year_dfs[yr] for yr in STATCAST_YEARS], ignore_index=True)
    return _build_profiles(combined)


# ── Load data — show a friendly loading screen on first boot ──────────────────
profiles_ready = "profiles" in st.session_state and st.session_state["profiles"] is not None

if not profiles_ready:
    # Show loading UI while data warms up
    st.markdown("""
    <div class="loading-wrap">
      <div class="loading-title">⚾ Loading Statcast Database</div>
      <div class="loading-sub">
        Fetching 2017–2024 pitch data from Baseball Savant in parallel.<br>
        This only happens <strong style="color:#8aadcc">once</strong> — every search after this is instant.
      </div>
    </div>
    """, unsafe_allow_html=True)

    prog = st.progress(0, text="Connecting to Baseball Savant…")
    stat = st.empty()

    stat.markdown(
        "<div style='text-align:center;font-family:monospace;font-size:12px;color:#3a6a8a'>"
        "Downloading 8 seasons in parallel…</div>",
        unsafe_allow_html=True,
    )

    # Trigger the cached load
    profiles = load_all_profiles()

    prog.progress(100, text="✓ All seasons loaded")
    stat.empty()

    st.session_state["profiles"] = profiles
    st.rerun()

profiles: pd.DataFrame = st.session_state["profiles"]

# ── Status chip in app bar area ───────────────────────────────────────────────
n_seasons  = profiles["year"].nunique()
n_pitchers = profiles["player_name"].nunique()
st.markdown(
    f"<div style='background:#0b1a2e;border-bottom:1px solid #1a3550;padding:6px 48px;"
    f"font-family:monospace;font-size:11px;color:#2a5a7a;display:flex;gap:24px'>"
    f"<span>✓ <b style='color:#3a8a5a'>{n_seasons} seasons loaded</b></span>"
    f"<span>·</span>"
    f"<span><b style='color:#3a8a5a'>{profiles.shape[0]:,}</b> pitcher-seasons indexed</span>"
    f"<span>·</span>"
    f"<span><b style='color:#3a8a5a'>{n_pitchers:,}</b> unique pitchers</span>"
    f"<span>·</span>"
...
if st.session_state.screen == "input":
    ...
    with main_col:
        ...
        st.markdown(
            "<div class='hb-note'>"
            "<b style='color:#8aadcc'>HB (in):</b> Enter raw horizontal break in inches. "
            "Positive = arm-side run, Negative = glove-side cut. "
            "Examples: RHP 4-seam tailing +14\", RHP slider cutting −4\", LHP changeup running +12\""
            "</div>",
            unsafe_allow_html=True,
        )
